import { vocabulary } from "./vocabulary.js?v=20260425-feature40";
import { phrases } from "./phrases.js?v=20260424-feature21";
import { topUpLanguageGroups } from "./language-topup.js?v=20260425-feature42";

const categoryMeta = {
  basic: {
    label: "初級",
    wordDescription: "はじめて学ぶ人向けの基本語です。短い会話で使いやすいものから確認できます。",
    phraseDescription: "あいさつ、返事、短い依頼で使いやすい基本フレーズです。"
  },
  practical: {
    label: "実用単語",
    wordDescription: "日常会話、説明、確認、生活場面で使いやすい実用語です。",
    phraseDescription: "予定調整、確認、相談、困りごとを伝える実用フレーズです。"
  },
  business: {
    label: "ビジネス",
    wordDescription: "会議、連絡、資料、進行管理で使いやすい仕事向けの語です。",
    phraseDescription: "会議、共有、確認、依頼、顧客対応で使いやすい仕事向けフレーズです。"
  },
  travel: {
    label: "旅行",
    wordDescription: "移動、宿泊、食事、買い物、トラブル対応で役立つ旅行語です。",
    phraseDescription: "空港、ホテル、レストラン、移動、緊急時でそのまま使える旅行フレーズです。"
  }
};

function buildEntries(items) {
  return items.map(([target, japanese, explanation]) => ({
    english: target,
    japanese,
    explanation
  }));
}

function isSingleTargetUnit(target, language) {
  const value = String(target || "").trim();
  if (!value) return false;

  if (language === "zh") {
    return !/[\s?？。.]/.test(value);
  }

  return !/\s/.test(value) && !/[?？。.]/.test(value);
}

const elementaryEnglishNounTargets = new Set([
  "apple",
  "banana",
  "orange",
  "grape",
  "pineapple",
  "tomato",
  "potato",
  "onion",
  "carrot",
  "egg",
  "milk",
  "bread",
  "rice",
  "fish",
  "notebook",
  "pen",
  "pencil",
  "eraser"
]);

function shouldKeepWordItem(item, language, categoryKey) {
  if (!isSingleTargetUnit(item.english, language) || isPhraseLikeWordLabel(item.japanese)) {
    return false;
  }

  if (language === "en" && categoryKey === "basic") {
    return !elementaryEnglishNounTargets.has(String(item.english || "").trim().toLowerCase());
  }

  return true;
}

function hashText(text) {
  let hash = 2166136261;
  for (const char of String(text || "").normalize("NFKC")) {
    hash ^= char.codePointAt(0);
    hash = Math.imul(hash, 16777619);
  }
  return (hash >>> 0).toString(36);
}

function makeItemId(kind, category, japanese) {
  return `${kind}:${category}:${hashText(japanese)}`;
}

function makeUniqueItemId(kind, category, target, japanese) {
  return `${kind}:${category}:${hashText(`${target}::${japanese}`)}`;
}

function dedupeEntries(items) {
  const seen = new Set();

  return items.filter(({ english, japanese }) => {
    const key = `${english.trim().toLowerCase()}::${japanese.trim()}`;
    if (seen.has(key)) {
      return false;
    }

    seen.add(key);
    return true;
  });
}

function decorateCollection(collection, kind, language) {
  return Object.fromEntries(Object.entries(collection).map(([key, category]) => [
    key,
    {
      ...category,
      words: category.words
        .filter((item) => kind !== "word" || shouldKeepWordItem(item, language, key))
        .map((item) => ({
          ...item,
          id: makeUniqueItemId(kind, key, item.english, item.japanese),
          meaningId: makeItemId(kind, key, item.japanese)
        }))
    }
  ]));
}

function createCollection(groups, kind, language) {
  return Object.fromEntries(Object.entries(categoryMeta).map(([key, meta]) => [
    key,
    {
      label: meta.label,
      description: kind === "word" ? meta.wordDescription : meta.phraseDescription,
      words: dedupeEntries(buildEntries(groups[key] || []))
        .filter((item) => kind !== "word" || shouldKeepWordItem(item, language, key))
        .map((item) => ({
          ...item,
          id: makeUniqueItemId(kind, key, item.english, item.japanese),
          meaningId: makeItemId(kind, key, item.japanese)
        }))
    }
  ]));
}

function mergeGroups(base, addition) {
  return Object.fromEntries(Object.keys(categoryMeta).map((key) => [
    key,
    [...(base[key] || []), ...(addition[key] || [])]
  ]));
}

function targetCountsFrom(collection) {
  return Object.fromEntries(Object.entries(collection).map(([key, category]) => [
    key,
    category.words.length
  ]));
}

const koreanWords = {
  basic: [
    ["나", "私"], ["너", "あなた"], ["우리", "私たち"], ["이것", "これ"], ["저것", "あれ"],
    ["여기", "ここ"], ["거기", "そこ"], ["누구", "だれ"], ["무엇", "何"], ["언제", "いつ"],
    ["어디", "どこ"], ["왜", "なぜ"], ["어떻게", "どうやって"], ["네", "はい"], ["아니요", "いいえ"],
    ["좋다", "良い"], ["크다", "大きい"], ["작다", "小さい"], ["많다", "多い"], ["조금", "少し"],
    ["오늘", "今日"], ["내일", "明日"], ["집", "家"], ["물", "水"], ["친구", "友だち"]
  ],
  practical: [
    ["도움", "助け / 手伝い"], ["시간", "時間"], ["문제", "問題"], ["방법", "方法"], ["이유", "理由"],
    ["생각", "考え"], ["약속", "約束 / 予定"], ["연락", "連絡"], ["확인", "確認"], ["설명", "説明"],
    ["질문", "質問"], ["대답", "答え"], ["연습", "練習"], ["기억", "記憶"], ["선택", "選択"],
    ["필요하다", "必要だ"], ["기다리다", "待つ"], ["보내다", "送る"], ["찾다", "探す"], ["잊다", "忘れる"],
    ["시작하다", "始める"], ["끝내다", "終える"], ["바꾸다", "変える"], ["비교하다", "比べる"], ["준비하다", "準備する"]
  ],
  business: [
    ["회의", "会議"], ["자료", "資料"], ["보고", "報告"], ["공유", "共有"], ["확인", "確認"],
    ["담당자", "担当者"], ["마감", "締切"], ["일정", "スケジュール"], ["승인", "承認"], ["제안", "提案"],
    ["고객", "顧客"], ["계약", "契約"], ["예산", "予算"], ["위험", "リスク"], ["우선순위", "優先順位"],
    ["진행", "進行"], ["수정", "修正"], ["최종본", "最終版"], ["요약", "要約"], ["검토", "検討"],
    ["연락하다", "連絡する"], ["공유하다", "共有する"], ["승인하다", "承認する"], ["준비하다", "準備する"], ["확정하다", "確定する"]
  ],
  travel: [
    ["공항", "空港"], ["역", "駅"], ["호텔", "ホテル"], ["방", "部屋"], ["예약", "予約"],
    ["여권", "パスポート"], ["짐", "荷物"], ["표", "チケット"], ["버스", "バス"], ["택시", "タクシー"],
    ["지도", "地図"], ["화장실", "トイレ"], ["식당", "レストラン"], ["메뉴", "メニュー"], ["계산", "会計"],
    ["영수증", "レシート"], ["약국", "薬局"], ["병원", "病院"], ["도움", "助け"], ["길", "道"],
    ["찾다", "探す"], ["사다", "買う"], ["타다", "乗る"], ["내리다", "降りる"], ["예약하다", "予約する"]
  ]
};

const koreanPhrases = {
  basic: [
    ["안녕하세요.", "こんにちは"], ["감사합니다.", "ありがとうございます"], ["죄송합니다.", "すみません / ごめんなさい"],
    ["괜찮아요.", "大丈夫です"], ["천천히 말해 주세요.", "ゆっくり話してください"],
    ["다시 말해 주세요.", "もう一度言ってください"], ["이게 뭐예요?", "これは何ですか"],
    ["잘 모르겠어요.", "よく分かりません"], ["도와주세요.", "助けてください"],
    ["잠깐만요.", "少し待ってください"], ["좋아요.", "いいです"], ["싫어요.", "嫌です"],
    ["저는 일본 사람입니다.", "私は日本人です"], ["한국어를 공부하고 있어요.", "韓国語を勉強しています"],
    ["이름이 뭐예요?", "名前は何ですか"], ["만나서 반가워요.", "会えてうれしいです"],
    ["괜찮습니까?", "大丈夫ですか"], ["네, 알겠습니다.", "はい、分かりました"],
    ["아직 잘 못해요.", "まだ上手にできません"], ["한 번 더 연습할게요.", "もう一度練習します"]
  ],
  practical: [
    ["조금 도와주실 수 있나요?", "少し手伝ってもらえますか"], ["시간이 조금 더 필요해요.", "もう少し時間が必要です"],
    ["먼저 확인해 볼게요.", "先に確認してみます"], ["예를 들어 설명해 주세요.", "例を出して説明してください"],
    ["이 부분이 이해가 안 돼요.", "この部分が分かりません"], ["다른 방법도 볼 수 있을까요?", "別の方法も見られますか"],
    ["더 자연스럽게 말하고 싶어요.", "もっと自然に言いたいです"], ["제 문장을 고쳐 주세요.", "私の文を直してください"],
    ["내일 다시 확인해요.", "明日もう一度確認しましょう"], ["기록으로 남기고 싶어요.", "記録に残したいです"],
    ["너무 어렵게 생각하지 맙시다.", "難しく考えすぎないようにしましょう"], ["상황에 따라 달라요.", "状況によって変わります"],
    ["무슨 뜻인지 정확히 알고 싶어요.", "正確な意味を知りたいです"], ["일단 하나씩 해 봅시다.", "まず一つずつやってみましょう"],
    ["나중에 알려 주세요.", "あとで教えてください"], ["글로 보내 주세요.", "文章で送ってください"],
    ["실수하지 않도록 확인하고 싶어요.", "間違えないように確認したいです"], ["조금 쉬어도 될까요?", "少し休んでもいいですか"],
    ["더 쉬운 표현이 있나요?", "もっと簡単な表現はありますか"], ["이 표현을 실제로 써 보고 싶어요.", "この表現を実際に使ってみたいです"]
  ],
  business: [
    ["진행 상황을 공유해 주세요.", "進捗を共有してください"], ["마감일을 확인합시다.", "締切を確認しましょう"],
    ["담당자가 누구인가요?", "担当者は誰ですか"], ["먼저 범위를 정합시다.", "先に範囲を決めましょう"],
    ["자료를 보내 드리겠습니다.", "資料をお送りします"], ["검토 후 연락드리겠습니다.", "確認後に連絡します"],
    ["승인이 필요합니다.", "承認が必要です"], ["간단히 요약해 주세요.", "簡単に要約してください"],
    ["고객에게 공유해 주세요.", "顧客に共有してください"], ["위험 요소를 확인합시다.", "リスクを確認しましょう"],
    ["우선순위를 정해야 합니다.", "優先順位を決める必要があります"], ["최종본을 보내 주세요.", "最終版を送ってください"],
    ["회의 후 메모를 공유하겠습니다.", "会議後にメモを共有します"], ["예산 범위를 확인할게요.", "予算の範囲を確認します"],
    ["일정을 조정할 수 있을까요?", "日程を調整できますか"], ["이 표현은 조금 애매합니다.", "この表現は少しあいまいです"],
    ["다음 행동을 명확히 합시다.", "次の行動を明確にしましょう"], ["계약 내용을 먼저 확인하겠습니다.", "契約内容を先に確認します"],
    ["오늘 안에 답변드리겠습니다.", "本日中に回答します"], ["추가 질문이 있으면 알려 주세요.", "追加質問があれば教えてください"]
  ],
  travel: [
    ["예약했습니다.", "予約しています"], ["이름은 ○○입니다.", "名前は○○です"],
    ["체크인하고 싶어요.", "チェックインしたいです"], ["이 방을 바꿀 수 있나요?", "この部屋を変更できますか"],
    ["택시를 불러 주세요.", "タクシーを呼んでください"], ["이 표는 어디서 사나요?", "このチケットはどこで買えますか"],
    ["이 버스가 공항에 가나요?", "このバスは空港に行きますか"], ["어디서 내려야 하나요?", "どこで降りればいいですか"],
    ["화장실이 어디예요?", "トイレはどこですか"], ["영수증 주세요.", "レシートをください"],
    ["카드로 결제할 수 있나요?", "カードで支払えますか"], ["맵지 않게 해 주세요.", "辛くしないでください"],
    ["알레르기가 있어요.", "アレルギーがあります"], ["약국이 근처에 있나요?", "近くに薬局はありますか"],
    ["길을 잃었어요.", "道に迷いました"], ["여기에 짐을 맡길 수 있나요?", "ここに荷物を預けられますか"],
    ["사진을 찍어 주실 수 있나요?", "写真を撮ってもらえますか"], ["와이파이 비밀번호를 알려 주세요.", "Wi-Fiのパスワードを教えてください"],
    ["추가 요금이 있나요?", "追加料金はありますか"], ["도움이 필요합니다.", "助けが必要です"]
  ]
};

const chineseWords = {
  basic: [
    ["我", "私"], ["你", "あなた"], ["他", "彼"], ["她", "彼女"], ["我们", "私たち"],
    ["这个", "これ"], ["那个", "あれ"], ["这里", "ここ"], ["那里", "そこ"], ["谁", "だれ"],
    ["什么", "何"], ["什么时候", "いつ"], ["哪里", "どこ"], ["为什么", "なぜ"], ["怎么", "どうやって"],
    ["是", "〜です"], ["不是", "〜ではありません"], ["好", "良い"], ["大", "大きい"], ["小", "小さい"],
    ["今天", "今日"], ["明天", "明日"], ["家", "家"], ["水", "水"], ["朋友", "友だち"]
  ],
  practical: [
    ["帮助", "助け / 手伝い"], ["时间", "時間"], ["问题", "問題"], ["方法", "方法"], ["理由", "理由"],
    ["想法", "考え"], ["预约", "予約 / 約束"], ["联系", "連絡"], ["确认", "確認"], ["说明", "説明"],
    ["提问", "質問する"], ["回答", "答え"], ["练习", "練習"], ["记住", "覚える"], ["选择", "選択"],
    ["需要", "必要だ"], ["等待", "待つ"], ["发送", "送る"], ["寻找", "探す"], ["忘记", "忘れる"],
    ["开始", "始める"], ["结束", "終える"], ["改变", "変える"], ["比较", "比べる"], ["准备", "準備する"]
  ],
  business: [
    ["会议", "会議"], ["资料", "資料"], ["报告", "報告"], ["共享", "共有"], ["确认", "確認"],
    ["负责人", "担当者"], ["截止日期", "締切"], ["日程", "スケジュール"], ["批准", "承認"], ["提案", "提案"],
    ["客户", "顧客"], ["合同", "契約"], ["预算", "予算"], ["风险", "リスク"], ["优先级", "優先順位"],
    ["进度", "進捗"], ["修改", "修正"], ["最终版", "最終版"], ["总结", "要約"], ["审查", "検討"],
    ["联系", "連絡する"], ["协作", "協力する"], ["签署", "署名する"], ["准备", "準備する"], ["确定", "確定する"]
  ],
  travel: [
    ["机场", "空港"], ["车站", "駅"], ["酒店", "ホテル"], ["房间", "部屋"], ["预订", "予約"],
    ["护照", "パスポート"], ["行李", "荷物"], ["票", "チケット"], ["公交车", "バス"], ["出租车", "タクシー"],
    ["地图", "地図"], ["洗手间", "トイレ"], ["餐厅", "レストラン"], ["菜单", "メニュー"], ["结账", "会計"],
    ["收据", "レシート"], ["药店", "薬局"], ["医院", "病院"], ["帮助", "助け"], ["路", "道"],
    ["找", "探す"], ["买", "買う"], ["坐车", "乗る"], ["下车", "降りる"], ["改签", "予約を変更する"]
  ]
};

const chinesePhrases = {
  basic: [
    ["你好。", "こんにちは"], ["谢谢。", "ありがとうございます"], ["对不起。", "ごめんなさい"],
    ["没关系。", "大丈夫です"], ["请说慢一点。", "ゆっくり話してください"],
    ["请再说一遍。", "もう一度言ってください"], ["这是什么？", "これは何ですか"],
    ["我不太明白。", "よく分かりません"], ["请帮帮我。", "助けてください"],
    ["请稍等。", "少し待ってください"], ["可以。", "いいです"], ["不可以。", "だめです"],
    ["我是日本人。", "私は日本人です"], ["我在学中文。", "中国語を勉強しています"],
    ["你叫什么名字？", "名前は何ですか"], ["很高兴认识你。", "会えてうれしいです"],
    ["你还好吗？", "大丈夫ですか"], ["好的，我明白了。", "はい、分かりました"],
    ["我还说得不好。", "まだ上手に話せません"], ["我再练习一次。", "もう一度練習します"]
  ],
  practical: [
    ["你能帮我一下吗？", "少し手伝ってもらえますか"], ["我需要多一点时间。", "もう少し時間が必要です"],
    ["我先确认一下。", "先に確認します"], ["请举个例子说明。", "例を出して説明してください"],
    ["我不明白这一部分。", "この部分が分かりません"], ["可以看看别的方法吗？", "別の方法も見られますか"],
    ["我想说得更自然。", "もっと自然に言いたいです"], ["请帮我修改这个句子。", "この文を直してください"],
    ["我们明天再确认吧。", "明日もう一度確認しましょう"], ["我想留下记录。", "記録に残したいです"],
    ["不要想得太复杂。", "難しく考えすぎないようにしましょう"], ["这要看情况。", "状況によります"],
    ["我想确认准确的意思。", "正確な意味を確認したいです"], ["我们一步一步来吧。", "一つずつ進めましょう"],
    ["请稍后告诉我。", "あとで教えてください"], ["请用文字发给我。", "文章で送ってください"],
    ["我想避免出错。", "間違えないようにしたいです"], ["可以休息一下吗？", "少し休んでもいいですか"],
    ["有更简单的说法吗？", "もっと簡単な言い方はありますか"], ["我想在实际对话中使用这个。", "実際の会話で使ってみたいです"]
  ],
  business: [
    ["请分享一下进度。", "進捗を共有してください"], ["我们确认一下截止日期。", "締切を確認しましょう"],
    ["负责人是谁？", "担当者は誰ですか"], ["先确认范围吧。", "先に範囲を確認しましょう"],
    ["我会发送资料。", "資料を送ります"], ["确认后我会联系您。", "確認後に連絡します"],
    ["这需要批准。", "承認が必要です"], ["请简单总结一下。", "簡単に要約してください"],
    ["请同步给客户。", "顧客に共有してください"], ["我们确认一下风险。", "リスクを確認しましょう"],
    ["需要先确定优先级。", "先に優先順位を決める必要があります"], ["请发送最终版。", "最終版を送ってください"],
    ["会后我会共享记录。", "会議後にメモを共有します"], ["我来确认预算范围。", "予算範囲を確認します"],
    ["可以调整日程吗？", "日程を調整できますか"], ["这个表达有点不明确。", "この表現は少しあいまいです"],
    ["让下一步更明确吧。", "次の行動を明確にしましょう"], ["我先确认合同内容。", "契約内容を先に確認します"],
    ["我今天之内回复。", "本日中に回答します"], ["如果有追加问题请告诉我。", "追加質問があれば教えてください"]
  ],
  travel: [
    ["我有预订。", "予約しています"], ["名字是○○。", "名前は○○です"],
    ["我想办理入住。", "チェックインしたいです"], ["可以换房间吗？", "部屋を変更できますか"],
    ["请帮我叫出租车。", "タクシーを呼んでください"], ["这个票在哪里买？", "このチケットはどこで買えますか"],
    ["这辆公交车去机场吗？", "このバスは空港に行きますか"], ["我应该在哪里下车？", "どこで降りればいいですか"],
    ["洗手间在哪里？", "トイレはどこですか"], ["请给我收据。", "レシートをください"],
    ["可以刷卡吗？", "カードで支払えますか"], ["请不要太辣。", "辛くしないでください"],
    ["我有过敏。", "アレルギーがあります"], ["附近有药店吗？", "近くに薬局はありますか"],
    ["我迷路了。", "道に迷いました"], ["这里可以寄存行李吗？", "ここに荷物を預けられますか"],
    ["可以帮我们拍张照吗？", "写真を撮ってもらえますか"], ["请告诉我Wi-Fi密码。", "Wi-Fiのパスワードを教えてください"],
    ["有额外费用吗？", "追加料金はありますか"], ["我需要帮助。", "助けが必要です"]
  ]
};

const frenchWords = {
  basic: [
    ["je", "私"], ["tu", "あなた"], ["il", "彼"], ["elle", "彼女"], ["nous", "私たち"],
    ["ceci", "これ"], ["cela", "あれ / それ"], ["ici", "ここ"], ["là", "そこ"], ["qui", "だれ"],
    ["quoi", "何"], ["quand", "いつ"], ["où", "どこ"], ["pourquoi", "なぜ"], ["comment", "どうやって"],
    ["oui", "はい"], ["non", "いいえ"], ["bon", "良い"], ["grand", "大きい"], ["petit", "小さい"],
    ["aujourd'hui", "今日"], ["demain", "明日"], ["maison", "家"], ["eau", "水"], ["ami", "友だち"]
  ],
  practical: [
    ["aide", "助け / 手伝い"], ["temps", "時間"], ["problème", "問題"], ["méthode", "方法"], ["raison", "理由"],
    ["idée", "考え"], ["rendez-vous", "約束 / 予約"], ["contact", "連絡"], ["confirmation", "確認"], ["explication", "説明"],
    ["question", "質問"], ["réponse", "答え"], ["pratique", "練習"], ["mémoire", "記憶"], ["choix", "選択"],
    ["nécessaire", "必要な"], ["attendre", "待つ"], ["envoyer", "送る"], ["chercher", "探す"], ["oublier", "忘れる"],
    ["commencer", "始める"], ["finir", "終える"], ["changer", "変える"], ["comparer", "比べる"], ["préparer", "準備する"]
  ],
  business: [
    ["réunion", "会議"], ["document", "資料"], ["rapport", "報告"], ["partage", "共有"], ["confirmation", "確認"],
    ["responsable", "担当者"], ["date limite", "締切"], ["planning", "スケジュール"], ["approbation", "承認"], ["proposition", "提案"],
    ["client", "顧客"], ["contrat", "契約"], ["budget", "予算"], ["risque", "リスク"], ["priorité", "優先順位"],
    ["avancement", "進捗"], ["révision", "修正"], ["version finale", "最終版"], ["résumé", "要約"], ["examen", "検討"],
    ["contacter", "連絡する"], ["partager", "共有する"], ["approuver", "承認する"], ["préparer", "準備する"], ["confirmer", "確定する"]
  ],
  travel: [
    ["aéroport", "空港"], ["gare", "駅"], ["hôtel", "ホテル"], ["chambre", "部屋"], ["réservation", "予約"],
    ["passeport", "パスポート"], ["bagage", "荷物"], ["billet", "チケット"], ["bus", "バス"], ["taxi", "タクシー"],
    ["carte", "地図"], ["toilettes", "トイレ"], ["restaurant", "レストラン"], ["menu", "メニュー"], ["addition", "会計"],
    ["reçu", "レシート"], ["pharmacie", "薬局"], ["hôpital", "病院"], ["aide", "助け"], ["chemin", "道"],
    ["chercher", "探す"], ["acheter", "買う"], ["prendre", "乗る"], ["descendre", "降りる"], ["réserver", "予約する"]
  ]
};

const frenchPhrases = {
  basic: [
    ["Bonjour.", "こんにちは"], ["Merci beaucoup.", "ありがとうございます"], ["Excusez-moi.", "すみません"],
    ["Ce n'est pas grave.", "大丈夫です"], ["Parlez lentement, s'il vous plaît.", "ゆっくり話してください"],
    ["Répétez, s'il vous plaît.", "もう一度言ってください"], ["Qu'est-ce que c'est ?", "これは何ですか"],
    ["Je ne comprends pas bien.", "よく分かりません"], ["Aidez-moi, s'il vous plaît.", "助けてください"],
    ["Un instant, s'il vous plaît.", "少し待ってください"], ["C'est bien.", "いいです"], ["Je n'aime pas ça.", "それは好きではありません"],
    ["Je suis japonais.", "私は日本人です"], ["J'étudie le français.", "フランス語を勉強しています"],
    ["Comment vous appelez-vous ?", "お名前は何ですか"], ["Enchanté.", "はじめまして"],
    ["Ça va ?", "大丈夫ですか"], ["D'accord, je comprends.", "はい、分かりました"],
    ["Je ne parle pas encore bien.", "まだ上手に話せません"], ["Je vais m'entraîner encore une fois.", "もう一度練習します"]
  ],
  practical: [
    ["Pouvez-vous m'aider un instant ?", "少し手伝ってもらえますか"], ["J'ai besoin d'un peu plus de temps.", "もう少し時間が必要です"],
    ["Je vais vérifier d'abord.", "先に確認します"], ["Pouvez-vous donner un exemple ?", "例を出してもらえますか"],
    ["Je ne comprends pas cette partie.", "この部分が分かりません"], ["Peut-on regarder une autre option ?", "別の選択肢も見られますか"],
    ["Je veux parler plus naturellement.", "もっと自然に話したいです"], ["Corrigez ma phrase, s'il vous plaît.", "私の文を直してください"],
    ["Revoyons cela demain.", "明日もう一度確認しましょう"], ["Je veux garder une trace.", "記録に残したいです"],
    ["Ne compliquons pas trop les choses.", "難しく考えすぎないようにしましょう"], ["Cela dépend de la situation.", "状況によります"],
    ["Je veux vérifier le sens exact.", "正確な意味を確認したいです"], ["Allons-y étape par étape.", "一つずつ進めましょう"],
    ["Dites-le-moi plus tard.", "あとで教えてください"], ["Envoyez-le-moi par écrit.", "文章で送ってください"],
    ["Je veux éviter les erreurs.", "間違えないようにしたいです"], ["Peut-on faire une petite pause ?", "少し休憩できますか"],
    ["Y a-t-il une façon plus simple de le dire ?", "もっと簡単な言い方はありますか"], ["Je veux l'utiliser dans une vraie conversation.", "実際の会話で使ってみたいです"]
  ],
  business: [
    ["Pouvez-vous partager l'avancement ?", "進捗を共有してもらえますか"], ["Confirmons la date limite.", "締切を確認しましょう"],
    ["Qui est responsable de cette tâche ?", "この作業の担当者は誰ですか"], ["Confirmons d'abord le périmètre.", "先に範囲を確認しましょう"],
    ["Je vous enverrai les documents.", "資料を送ります"], ["Je vous contacterai après vérification.", "確認後に連絡します"],
    ["Une approbation est nécessaire.", "承認が必要です"], ["Pouvez-vous résumer brièvement ?", "簡単に要約してもらえますか"],
    ["Tenez le client informé.", "顧客に共有し続けてください"], ["Vérifions les risques.", "リスクを確認しましょう"],
    ["Nous devons fixer les priorités.", "優先順位を決める必要があります"], ["Envoyez la version finale, s'il vous plaît.", "最終版を送ってください"],
    ["Je partagerai les notes après la réunion.", "会議後にメモを共有します"], ["Je vais confirmer le budget.", "予算を確認します"],
    ["Peut-on ajuster le planning ?", "日程を調整できますか"], ["Cette formulation est un peu vague.", "この表現は少しあいまいです"],
    ["Clarifions la prochaine action.", "次の行動を明確にしましょう"], ["Je vais d'abord vérifier le contrat.", "先に契約内容を確認します"],
    ["Je répondrai d'ici la fin de la journée.", "本日中に回答します"], ["Dites-moi si vous avez d'autres questions.", "追加質問があれば教えてください"]
  ],
  travel: [
    ["J'ai une réservation.", "予約しています"], ["La réservation est au nom de ○○.", "予約名は○○です"],
    ["Je voudrais faire le check-in.", "チェックインしたいです"], ["Puis-je changer de chambre ?", "部屋を変更できますか"],
    ["Pouvez-vous appeler un taxi ?", "タクシーを呼んでもらえますか"], ["Où puis-je acheter ce billet ?", "このチケットはどこで買えますか"],
    ["Ce bus va à l'aéroport ?", "このバスは空港に行きますか"], ["Où dois-je descendre ?", "どこで降りればいいですか"],
    ["Où sont les toilettes ?", "トイレはどこですか"], ["Un reçu, s'il vous plaît.", "レシートをください"],
    ["Puis-je payer par carte ?", "カードで支払えますか"], ["Pas trop épicé, s'il vous plaît.", "辛くしないでください"],
    ["J'ai une allergie.", "アレルギーがあります"], ["Y a-t-il une pharmacie près d'ici ?", "近くに薬局はありますか"],
    ["Je suis perdu.", "道に迷いました"], ["Puis-je laisser mes bagages ici ?", "ここに荷物を預けられますか"],
    ["Pouvez-vous nous prendre en photo ?", "写真を撮ってもらえますか"], ["Quel est le mot de passe Wi-Fi ?", "Wi-Fiのパスワードは何ですか"],
    ["Y a-t-il des frais supplémentaires ?", "追加料金はありますか"], ["J'ai besoin d'aide.", "助けが必要です"]
  ]
};

const koreanWordBoost = {
  basic: [
    ["사람", "人"], ["아이", "子ども"], ["학생", "学生"], ["선생님", "先生"], ["어머니", "母"],
    ["아버지", "父"], ["가족", "家族"], ["학교", "学校"], ["회사", "会社"], ["가게", "店"],
    ["책", "本"], ["공책", "ノート"], ["펜", "ペン"], ["의자", "椅子"], ["책상", "机"],
    ["문", "ドア"], ["창문", "窓"], ["차", "車 / お茶"], ["커피", "コーヒー"], ["밥", "ご飯 / 食事"],
    ["빵", "パン"], ["사과", "りんご"], ["날씨", "天気"], ["비", "雨"], ["눈", "雪 / 目"],
    ["아침", "朝"], ["점심", "昼食"], ["저녁", "夕方 / 夕食"], ["밤", "夜"], ["지금", "今"],
    ["어제", "昨日"], ["주말", "週末"], ["월요일", "月曜日"], ["전화", "電話"], ["사진", "写真"],
    ["말", "言葉 / 話"], ["이름", "名前"], ["나라", "国"], ["도시", "都市"], ["한국", "韓国"],
    ["일본", "日本"], ["좋아하다", "好きだ"], ["먹다", "食べる"], ["마시다", "飲む"], ["가다", "行く"],
    ["오다", "来る"], ["보다", "見る"], ["듣다", "聞く"], ["말하다", "話す"], ["공부하다", "勉強する"]
  ],
  practical: [
    ["생활", "生活"], ["습관", "習慣"], ["건강", "健康"], ["몸", "体"], ["머리", "頭"],
    ["마음", "心 / 気持ち"], ["기분", "気分"], ["걱정", "心配"], ["실수", "ミス"], ["차이", "違い"],
    ["예", "例"], ["뜻", "意味"], ["표현", "表現"], ["발음", "発音"], ["소리", "音"],
    ["문장", "文"], ["단어", "単語"], ["내용", "内容"], ["정보", "情報"], ["주소", "住所"],
    ["번호", "番号"], ["비밀번호", "パスワード"], ["화면", "画面"], ["파일", "ファイル"], ["메시지", "メッセージ"],
    ["앱", "アプリ"], ["인터넷", "インターネット"], ["충전", "充電"], ["배터리", "バッテリー"], ["속도", "速度"],
    ["가능하다", "可能だ"], ["어렵다", "難しい"], ["쉽다", "簡単だ"], ["중요하다", "重要だ"], ["자연스럽다", "自然だ"],
    ["필요 없다", "必要ない"], ["도착하다", "到着する"], ["기록하다", "記録する"], ["확인하다", "確認する"], ["설명하다", "説明する"],
    ["연습하다", "練習する"], ["질문하다", "質問する"], ["대답하다", "答える"], ["기억하다", "覚えている"], ["이해하다", "理解する"],
    ["추천하다", "おすすめする"], ["고치다", "直す"], ["쉬다", "休む"], ["정하다", "決める"], ["계속하다", "続ける"]
  ],
  business: [
    ["업무", "業務"], ["프로젝트", "プロジェクト"], ["팀", "チーム"], ["부서", "部署"], ["동료", "同僚"],
    ["상사", "上司"], ["직원", "社員"], ["거래처", "取引先"], ["문의", "問い合わせ"], ["요청", "依頼"],
    ["답변", "回答"], ["이메일", "メール"], ["첨부파일", "添付ファイル"], ["양식", "フォーム"], ["회의실", "会議室"],
    ["목표", "目標"], ["성과", "成果"], ["문서", "文書"], ["기획", "企画"], ["운영", "運用"],
    ["문제점", "問題点"], ["개선", "改善"], ["비용", "費用"], ["매출", "売上"], ["품질", "品質"],
    ["일정표", "予定表"], ["공개", "公開"], ["발표", "発表"], ["준비물", "準備物"], ["결정", "決定"],
    ["검토하다", "検討する"], ["보고하다", "報告する"], ["요청하다", "依頼する"], ["수정하다", "修正する"], ["공개하다", "公開する"],
    ["논의하다", "議論する"], ["협의하다", "協議する"], ["제출하다", "提出する"], ["완료하다", "完了する"], ["연기하다", "延期する"],
    ["취소하다", "キャンセルする"], ["관리하다", "管理する"], ["공유받다", "共有してもらう"], ["정리하다", "整理する"], ["추가하다", "追加する"],
    ["삭제하다", "削除する"], ["확인되다", "確認される"], ["필요합니다", "必要です"], ["가능합니다", "可能です"], ["어렵습니다", "難しいです"]
  ],
  travel: [
    ["출발", "出発"], ["도착", "到着"], ["입구", "入口"], ["출구", "出口"], ["매표소", "切符売り場"],
    ["승강장", "乗り場 / ホーム"], ["지하철", "地下鉄"], ["기차", "電車 / 列車"], ["비행기", "飛行機"], ["좌석", "座席"],
    ["통로", "通路"], ["창가", "窓側"], ["출입국", "出入国"], ["세관", "税関"], ["숙소", "宿泊先"],
    ["프런트", "フロント"], ["열쇠", "鍵"], ["수건", "タオル"], ["샤워", "シャワー"], ["에어컨", "エアコン"],
    ["난방", "暖房"], ["조식", "朝食"], ["주문", "注文"], ["물수건", "おしぼり"], ["젓가락", "箸"],
    ["포장", "持ち帰り"], ["가격", "価格"], ["할인", "割引"], ["현금", "現金"], ["카드", "カード"],
    ["잔돈", "おつり"], ["기념품", "お土産"], ["사이즈", "サイズ"], ["색깔", "色"], ["교환", "交換"],
    ["환불", "返金"], ["분실물", "忘れ物"], ["경찰서", "警察署"], ["응급실", "救急外来"], ["보험", "保険"],
    ["예약 확인", "予約確認"], ["길 안내", "道案内"], ["관광지", "観光地"], ["입장권", "入場券"], ["운영 시간", "営業時間"],
    ["닫다", "閉まる"], ["열다", "開く"], ["걷다", "歩く"], ["갈아타다", "乗り換える"], ["예약을 변경하다", "予約を変更する"]
  ]
};

const koreanPhraseBoost = {
  basic: [
    ["오늘 시간이 있어요?", "今日時間はありますか"], ["저는 조금 피곤해요.", "私は少し疲れています"], ["물 한 잔 주세요.", "水を一杯ください"],
    ["이거 주세요.", "これをください"], ["천천히 해도 괜찮아요.", "ゆっくりで大丈夫です"], ["지금 가도 돼요?", "今行ってもいいですか"],
    ["여기에 앉아도 돼요?", "ここに座ってもいいですか"], ["사진을 봐 주세요.", "写真を見てください"], ["제 이름은 ○○입니다.", "私の名前は○○です"],
    ["집에 가고 싶어요.", "家に帰りたいです"], ["친구를 기다리고 있어요.", "友だちを待っています"], ["오늘 날씨가 좋아요.", "今日は天気がいいです"],
    ["이 음식은 맛있어요.", "この料理はおいしいです"], ["커피를 마시고 싶어요.", "コーヒーを飲みたいです"], ["다음에 또 만나요.", "次にまた会いましょう"],
    ["무슨 말이에요?", "どういう意味ですか"], ["여기가 맞아요?", "ここで合っていますか"], ["잠깐 전화해도 돼요?", "少し電話してもいいですか"],
    ["오늘은 여기까지 할게요.", "今日はここまでにします"], ["저도 그렇게 생각해요.", "私もそう思います"], ["아직 준비가 안 됐어요.", "まだ準備できていません"],
    ["먼저 해 보세요.", "先にやってみてください"], ["같이 가요.", "一緒に行きましょう"], ["조심하세요.", "気をつけてください"], ["괜찮으면 알려 주세요.", "大丈夫なら教えてください"]
  ],
  practical: [
    ["제가 이해한 게 맞나요?", "私の理解で合っていますか"], ["다시 한번 정리해 볼게요.", "もう一度整理してみます"], ["이 표현은 자연스러운가요?", "この表現は自然ですか"],
    ["더 쉬운 예문이 필요해요.", "もっと簡単な例文が必要です"], ["제가 고친 문장을 확인해 주세요.", "私が直した文を確認してください"], ["이 단어를 어떻게 써요?", "この単語をどう使いますか"],
    ["오늘 배운 내용을 복습하고 싶어요.", "今日学んだ内容を復習したいです"], ["이건 나중에 다시 볼게요.", "これはあとでまた見ます"], ["잠깐 생각할 시간을 주세요.", "少し考えさせてください"],
    ["제가 실수한 부분을 알려 주세요.", "私が間違えた部分を教えてください"], ["답만 말고 이유도 알고 싶어요.", "答えだけでなく理由も知りたいです"], ["두 표현의 차이를 설명해 주세요.", "二つの表現の違いを説明してください"],
    ["상황에 맞게 말하고 싶어요.", "場面に合うように言いたいです"], ["기록을 저장하고 계속 공부할게요.", "記録を保存して続けて勉強します"], ["오늘 목표를 채우고 싶어요.", "今日の目標を達成したいです"],
    ["조금 더 자연스럽게 바꿔 주세요.", "もう少し自然に変えてください"], ["발음을 다시 들려 주세요.", "発音をもう一度聞かせてください"], ["이 문장은 너무 딱딱해요.", "この文は硬すぎます"],
    ["일상 대화에서 쓸 수 있나요?", "日常会話で使えますか"], ["제가 직접 문장을 만들어 볼게요.", "自分で文を作ってみます"], ["헷갈리는 단어를 다시 연습할게요.", "紛らわしい単語をもう一度練習します"],
    ["이건 급하지 않아요.", "これは急ぎではありません"], ["먼저 중요한 것부터 할게요.", "先に重要なものからやります"], ["비슷한 표현도 알려 주세요.", "似た表現も教えてください"], ["오늘은 듣기 중심으로 할게요.", "今日は聞く練習を中心にします"]
  ],
  business: [
    ["오늘 회의 안건을 확인하겠습니다.", "今日の会議議題を確認します"], ["자료는 회의 전에 공유하겠습니다.", "資料は会議前に共有します"], ["담당자를 먼저 확인해 주세요.", "担当者を先に確認してください"],
    ["이 건은 고객에게 확인이 필요합니다.", "この件は顧客への確認が必要です"], ["마감일을 하루 미룰 수 있을까요?", "締切を1日延ばせますか"], ["최신 파일로 다시 보내 주세요.", "最新版のファイルで送り直してください"],
    ["결정된 내용을 짧게 정리하겠습니다.", "決定内容を短くまとめます"], ["리스크가 있으면 바로 알려 주세요.", "リスクがあればすぐ知らせてください"], ["예산 범위 안에서 진행하겠습니다.", "予算範囲内で進めます"],
    ["우선순위를 다시 정해야 합니다.", "優先順位を決め直す必要があります"], ["이 표현은 고객에게 너무 강하게 들립니다.", "この表現は顧客には強すぎます"], ["다음 행동을 명확히 적어 주세요.", "次の行動を明確に書いてください"],
    ["검토 후 승인 요청을 올리겠습니다.", "確認後に承認依頼を出します"], ["오늘 중으로 1차 답변을 드리겠습니다.", "本日中に一次回答します"], ["회의록은 제가 작성하겠습니다.", "議事録は私が作成します"],
    ["문제가 생기면 즉시 공유하겠습니다.", "問題が起きたらすぐ共有します"], ["이 부분은 내부에서 논의하겠습니다.", "この部分は社内で議論します"], ["계약 조건을 다시 확인해야 합니다.", "契約条件を再確認する必要があります"],
    ["일정 변경 가능 여부를 확인하겠습니다.", "日程変更が可能か確認します"], ["최종본은 내일 오전에 보내겠습니다.", "最終版は明日の午前に送ります"], ["담당 범위를 분명히 합시다.", "担当範囲を明確にしましょう"],
    ["요청 사항을 목록으로 정리했습니다.", "依頼事項をリストに整理しました"], ["이 내용은 메일로 남기겠습니다.", "この内容はメールに残します"], ["다음 회의에서 다시 다루겠습니다.", "次の会議で再度扱います"], ["추가 비용이 발생할 수 있습니다.", "追加費用が発生する可能性があります"]
  ],
  travel: [
    ["공항까지 얼마나 걸려요?", "空港までどのくらいかかりますか"], ["이 표로 오늘 탈 수 있나요?", "このチケットで今日乗れますか"], ["가장 가까운 역이 어디예요?", "一番近い駅はどこですか"],
    ["체크아웃 시간을 늦출 수 있나요?", "チェックアウト時間を遅らせられますか"], ["방에 뜨거운 물이 안 나와요.", "部屋でお湯が出ません"], ["에어컨이 작동하지 않아요.", "エアコンが動きません"],
    ["조용한 방으로 바꿀 수 있나요?", "静かな部屋に変えられますか"], ["이 주소로 가 주세요.", "この住所へ行ってください"], ["여기서 사진을 찍어도 되나요?", "ここで写真を撮ってもいいですか"],
    ["추천 메뉴가 뭐예요?", "おすすめメニューは何ですか"], ["고기를 빼 주세요.", "肉を抜いてください"], ["포장해 주세요.", "持ち帰りにしてください"],
    ["계산을 따로 할 수 있나요?", "会計を別々にできますか"], ["잔돈을 받을 수 있나요?", "おつりをもらえますか"], ["이 물건을 교환하고 싶어요.", "この商品を交換したいです"],
    ["영수증을 다시 받을 수 있나요?", "レシートをもう一度もらえますか"], ["여권을 잃어버렸어요.", "パスポートをなくしました"], ["도움이 필요한 상황입니다.", "助けが必要な状況です"],
    ["근처 병원에 가고 싶어요.", "近くの病院に行きたいです"], ["약을 살 수 있는 곳이 있나요?", "薬を買える場所はありますか"], ["짐을 저녁까지 맡기고 싶어요.", "荷物を夕方まで預けたいです"],
    ["이 길은 밤에도 안전한가요?", "この道は夜でも安全ですか"], ["입장권은 어디서 사요?", "入場券はどこで買いますか"], ["오늘 영업하나요?", "今日は営業していますか"], ["현금 없이 결제할 수 있나요?", "現金なしで支払えますか"]
  ]
};

const chineseWordBoost = {
  basic: [
    ["人", "人"], ["孩子", "子ども"], ["学生", "学生"], ["老师", "先生"], ["妈妈", "母"],
    ["爸爸", "父"], ["家人", "家族"], ["学校", "学校"], ["公司", "会社"], ["商店", "店"],
    ["书", "本"], ["本子", "ノート"], ["笔", "ペン"], ["椅子", "椅子"], ["桌子", "机"],
    ["门", "ドア"], ["窗户", "窓"], ["茶", "お茶"], ["咖啡", "コーヒー"], ["米饭", "ご飯"],
    ["面包", "パン"], ["苹果", "りんご"], ["天气", "天気"], ["雨", "雨"], ["雪", "雪"],
    ["早上", "朝"], ["中午", "昼"], ["晚上", "夜"], ["现在", "今"], ["昨天", "昨日"],
    ["周末", "週末"], ["星期一", "月曜日"], ["电话", "電話"], ["照片", "写真"], ["名字", "名前"],
    ["国家", "国"], ["城市", "都市"], ["中国", "中国"], ["日本", "日本"], ["喜欢", "好きだ"],
    ["吃", "食べる"], ["喝", "飲む"], ["去", "行く"], ["来", "来る"], ["看", "見る"],
    ["听", "聞く"], ["说", "話す"], ["学习", "勉強する"], ["写", "書く"], ["读", "読む"]
  ],
  practical: [
    ["生活", "生活"], ["习惯", "習慣"], ["健康", "健康"], ["身体", "体"], ["心情", "気分"],
    ["担心", "心配"], ["错误", "ミス"], ["区别", "違い"], ["例子", "例"], ["意思", "意味"],
    ["表达", "表現"], ["发音", "発音"], ["声音", "音"], ["句子", "文"], ["单词", "単語"],
    ["内容", "内容"], ["信息", "情報"], ["地址", "住所"], ["号码", "番号"], ["密码", "パスワード"],
    ["屏幕", "画面"], ["文件", "ファイル"], ["消息", "メッセージ"], ["应用", "アプリ"], ["网络", "インターネット"],
    ["充电", "充電"], ["电池", "バッテリー"], ["速度", "速度"], ["可能", "可能"], ["困难", "難しい"],
    ["简单", "簡単"], ["重要", "重要"], ["自然", "自然"], ["不需要", "必要ない"], ["到达", "到着する"],
    ["记录", "記録する"], ["解释", "説明する"], ["提问", "質問する"], ["理解", "理解する"], ["推荐", "おすすめする"],
    ["修改", "直す"], ["休息", "休む"], ["决定", "決める"], ["继续", "続ける"], ["复习", "復習する"],
    ["整理", "整理する"], ["保存", "保存する"], ["删除", "削除する"], ["下载", "ダウンロードする"], ["上传", "アップロードする"],
    ["提醒", "思い出させる / 注意する"]
  ],
  business: [
    ["工作", "仕事"], ["项目", "プロジェクト"], ["团队", "チーム"], ["部门", "部署"], ["同事", "同僚"],
    ["上司", "上司"], ["员工", "社員"], ["合作方", "取引先"], ["咨询", "問い合わせ"], ["请求", "依頼"],
    ["回复", "返信 / 回答"], ["邮件", "メール"], ["附件", "添付ファイル"], ["表格", "フォーム"], ["会议室", "会議室"],
    ["目标", "目標"], ["成果", "成果"], ["文件", "文書"], ["计划", "計画"], ["运营", "運用"],
    ["问题点", "問題点"], ["改善", "改善"], ["费用", "費用"], ["销售额", "売上"], ["质量", "品質"],
    ["时间表", "予定表"], ["发布", "公開"], ["发表", "発表"], ["准备物", "準備物"], ["决定", "決定"],
    ["核对", "照合する"], ["汇报", "報告する"], ["安排", "手配する"], ["更正", "修正する"], ["上线", "公開する"],
    ["讨论", "議論する"], ["协商", "協議する"], ["提交", "提出する"], ["完成", "完了する"], ["延期", "延期する"],
    ["取消", "キャンセルする"], ["管理", "管理する"], ["整理", "整理する"], ["追加", "追加する"], ["删除", "削除する"],
    ["签字", "署名する"], ["可以", "可能です"], ["比较困难", "難しいです"], ["需要确认", "確認が必要"], ["优先处理", "優先対応"]
  ],
  travel: [
    ["出发", "出発"], ["到达", "到着"], ["入口", "入口"], ["出口", "出口"], ["售票处", "切符売り場"],
    ["站台", "ホーム"], ["地铁", "地下鉄"], ["火车", "列車"], ["飞机", "飛行機"], ["座位", "座席"],
    ["过道", "通路"], ["靠窗", "窓側"], ["海关", "税関"], ["住宿", "宿泊先"], ["前台", "フロント"],
    ["钥匙", "鍵"], ["毛巾", "タオル"], ["淋浴", "シャワー"], ["空调", "エアコン"], ["暖气", "暖房"],
    ["早餐", "朝食"], ["点餐", "注文"], ["筷子", "箸"], ["打包", "持ち帰り"], ["价格", "価格"],
    ["折扣", "割引"], ["现金", "現金"], ["零钱", "おつり"], ["纪念品", "お土産"], ["尺码", "サイズ"],
    ["颜色", "色"], ["交换", "交換"], ["退款", "返金"], ["失物", "忘れ物"], ["警察局", "警察署"],
    ["急诊", "救急外来"], ["保险", "保険"], ["确认预订", "予約確認"], ["问路", "道案内"], ["景点", "観光地"],
    ["门票", "入場券"], ["营业时间", "営業時間"], ["关门", "閉まる"], ["开门", "開く"], ["走路", "歩く"],
    ["换乘", "乗り換える"], ["入住", "チェックインする"], ["直达", "直通"], ["附近", "近く"], ["安全", "安全"]
  ]
};

const chinesePhraseBoost = {
  basic: [
    ["今天有时间吗？", "今日時間はありますか"], ["我有点累。", "少し疲れています"], ["请给我一杯水。", "水を一杯ください"],
    ["我要这个。", "これをください"], ["慢慢来没关系。", "ゆっくりで大丈夫です"], ["我现在可以走吗？", "今行ってもいいですか"],
    ["我可以坐这里吗？", "ここに座ってもいいですか"], ["请看这张照片。", "この写真を見てください"], ["我的名字是○○。", "私の名前は○○です"],
    ["我想回家。", "家に帰りたいです"], ["我在等朋友。", "友だちを待っています"], ["今天天气很好。", "今日は天気がいいです"],
    ["这个菜很好吃。", "この料理はおいしいです"], ["我想喝咖啡。", "コーヒーを飲みたいです"], ["下次再见。", "次にまた会いましょう"],
    ["这是什么意思？", "これはどういう意味ですか"], ["这里对吗？", "ここで合っていますか"], ["我可以打个电话吗？", "少し電話してもいいですか"],
    ["今天就到这里。", "今日はここまでにします"], ["我也这么想。", "私もそう思います"], ["我还没准备好。", "まだ準備できていません"],
    ["你先试试看。", "先にやってみてください"], ["我们一起去吧。", "一緒に行きましょう"], ["请小心。", "気をつけてください"], ["如果可以，请告诉我。", "大丈夫なら教えてください"]
  ],
  practical: [
    ["我的理解对吗？", "私の理解で合っていますか"], ["我再整理一下。", "もう一度整理します"], ["这个表达自然吗？", "この表現は自然ですか"],
    ["我需要更简单的例句。", "もっと簡単な例文が必要です"], ["请确认我修改的句子。", "私が直した文を確認してください"], ["这个词怎么用？", "この単語をどう使いますか"],
    ["我想复习今天学的内容。", "今日学んだ内容を復習したいです"], ["这个我以后再看。", "これはあとでまた見ます"], ["请给我一点时间想想。", "少し考えさせてください"],
    ["请告诉我哪里错了。", "どこを間違えたか教えてください"], ["我不仅想知道答案，也想知道原因。", "答えだけでなく理由も知りたいです"], ["请说明这两个表达的区别。", "二つの表現の違いを説明してください"],
    ["我想根据场景来说。", "場面に合わせて言いたいです"], ["我会保存记录继续学习。", "記録を保存して続けて学習します"], ["我想完成今天的目标。", "今日の目標を達成したいです"],
    ["请改得更自然一点。", "もう少し自然に直してください"], ["请再播放一次发音。", "発音をもう一度再生してください"], ["这个句子太生硬了。", "この文は硬すぎます"],
    ["日常对话中可以用吗？", "日常会話で使えますか"], ["我自己造个句子试试。", "自分で文を作ってみます"], ["我想再练习容易混淆的词。", "紛らわしい単語をもう一度練習します"],
    ["这件事不急。", "これは急ぎではありません"], ["我先做重要的部分。", "先に重要な部分からやります"], ["请也告诉我类似的表达。", "似た表現も教えてください"], ["今天我想重点练听力。", "今日は聞く練習を中心にします"]
  ],
  business: [
    ["我确认一下今天的会议议题。", "今日の会議議題を確認します"], ["资料会在会议前共享。", "資料は会議前に共有します"], ["请先确认负责人。", "担当者を先に確認してください"],
    ["这件事需要和客户确认。", "この件は顧客確認が必要です"], ["截止日期可以推迟一天吗？", "締切を1日延ばせますか"], ["请用最新版文件重新发送。", "最新版のファイルで送り直してください"],
    ["我会简单整理决定事项。", "決定事項を短く整理します"], ["如果有风险，请马上告诉我。", "リスクがあればすぐ教えてください"], ["我会在预算范围内推进。", "予算範囲内で進めます"],
    ["我们需要重新确定优先级。", "優先順位を決め直す必要があります"], ["这个说法对客户来说太强了。", "この表現は顧客には強すぎます"], ["请写清楚下一步行动。", "次の行動を明確に書いてください"],
    ["确认后我会提交审批。", "確認後に承認依頼を出します"], ["今天之内我会先回复。", "本日中に一次回答します"], ["会议记录由我来写。", "議事録は私が作成します"],
    ["如果出现问题，我会马上共享。", "問題が起きたらすぐ共有します"], ["这部分我们内部讨论。", "この部分は社内で議論します"], ["需要重新确认合同条件。", "契約条件を再確認する必要があります"],
    ["我确认一下能否调整日程。", "日程変更が可能か確認します"], ["最终版明天上午发送。", "最終版は明日の午前に送ります"], ["明确一下负责范围吧。", "担当範囲を明確にしましょう"],
    ["我把请求事项整理成了列表。", "依頼事項をリストに整理しました"], ["这个内容我会用邮件留下记录。", "この内容はメールに残します"], ["下次会议再讨论这个。", "次の会議で再度扱います"], ["可能会产生追加费用。", "追加費用が発生する可能性があります"]
  ],
  travel: [
    ["到机场需要多长时间？", "空港までどのくらいかかりますか"], ["这张票今天可以用吗？", "このチケットは今日使えますか"], ["最近的车站在哪里？", "一番近い駅はどこですか"],
    ["可以延迟退房吗？", "チェックアウトを遅らせられますか"], ["房间没有热水。", "部屋でお湯が出ません"], ["空调不能用。", "エアコンが動きません"],
    ["可以换一个安静的房间吗？", "静かな部屋に変えられますか"], ["请去这个地址。", "この住所へ行ってください"], ["这里可以拍照吗？", "ここで写真を撮ってもいいですか"],
    ["有什么推荐菜？", "おすすめ料理は何ですか"], ["请不要放肉。", "肉を入れないでください"], ["请打包。", "持ち帰りにしてください"],
    ["可以分开付款吗？", "会計を別々にできますか"], ["可以找零钱吗？", "おつりをもらえますか"], ["我想换这个商品。", "この商品を交換したいです"],
    ["可以再给我一张收据吗？", "レシートをもう一度もらえますか"], ["我的护照丢了。", "パスポートをなくしました"], ["我现在需要帮助。", "今助けが必要です"],
    ["我想去附近的医院。", "近くの病院に行きたいです"], ["哪里可以买药？", "どこで薬を買えますか"], ["我想把行李寄存到晚上。", "荷物を夜まで預けたいです"],
    ["这条路晚上安全吗？", "この道は夜でも安全ですか"], ["门票在哪里买？", "入場券はどこで買いますか"], ["今天营业吗？", "今日は営業していますか"], ["可以不用现金支付吗？", "現金なしで支払えますか"]
  ]
};

const frenchWordBoost = {
  basic: [
    ["personne", "人"], ["enfant", "子ども"], ["étudiant", "学生"], ["professeur", "先生"], ["mère", "母"],
    ["père", "父"], ["famille", "家族"], ["école", "学校"], ["entreprise", "会社"], ["magasin", "店"],
    ["livre", "本"], ["cahier", "ノート"], ["stylo", "ペン"], ["chaise", "椅子"], ["table", "机 / テーブル"],
    ["porte", "ドア"], ["fenêtre", "窓"], ["thé", "お茶"], ["café", "コーヒー"], ["riz", "ご飯"],
    ["pain", "パン"], ["pomme", "りんご"], ["temps", "天気 / 時間"], ["pluie", "雨"], ["neige", "雪"],
    ["matin", "朝"], ["midi", "昼"], ["soir", "夕方"], ["nuit", "夜"], ["maintenant", "今"],
    ["hier", "昨日"], ["week-end", "週末"], ["lundi", "月曜日"], ["téléphone", "電話"], ["photo", "写真"],
    ["nom", "名前"], ["pays", "国"], ["ville", "都市"], ["France", "フランス"], ["Japon", "日本"],
    ["aimer", "好きだ"], ["manger", "食べる"], ["boire", "飲む"], ["aller", "行く"], ["venir", "来る"],
    ["voir", "見る"], ["écouter", "聞く"], ["parler", "話す"], ["étudier", "勉強する"], ["écrire", "書く"]
  ],
  practical: [
    ["vie", "生活"], ["habitude", "習慣"], ["santé", "健康"], ["corps", "体"], ["humeur", "気分"],
    ["inquiétude", "心配"], ["erreur", "ミス"], ["différence", "違い"], ["exemple", "例"], ["sens", "意味"],
    ["expression", "表現"], ["prononciation", "発音"], ["son", "音"], ["phrase", "文"], ["mot", "単語"],
    ["contenu", "内容"], ["information", "情報"], ["adresse", "住所"], ["numéro", "番号"], ["mot de passe", "パスワード"],
    ["écran", "画面"], ["fichier", "ファイル"], ["message", "メッセージ"], ["application", "アプリ"], ["Internet", "インターネット"],
    ["charge", "充電"], ["batterie", "バッテリー"], ["vitesse", "速度"], ["possible", "可能な"], ["difficile", "難しい"],
    ["facile", "簡単な"], ["important", "重要な"], ["naturel", "自然な"], ["inutile", "不要な"], ["arriver", "到着する"],
    ["noter", "記録する"], ["expliquer", "説明する"], ["demander", "質問する"], ["comprendre", "理解する"], ["recommander", "おすすめする"],
    ["corriger", "直す"], ["se reposer", "休む"], ["décider", "決める"], ["continuer", "続ける"], ["répéter", "繰り返す"],
    ["organiser", "整理する"], ["sauvegarder", "保存する"], ["supprimer", "削除する"], ["télécharger", "ダウンロードする"], ["envoyer", "送る"],
    ["rappeler", "思い出させる / 電話をかけ直す"]
  ],
  business: [
    ["travail", "仕事"], ["projet", "プロジェクト"], ["équipe", "チーム"], ["service", "部署"], ["collègue", "同僚"],
    ["supérieur", "上司"], ["employé", "社員"], ["partenaire", "取引先"], ["demande", "問い合わせ / 依頼"], ["réponse", "回答"],
    ["e-mail", "メール"], ["pièce jointe", "添付ファイル"], ["formulaire", "フォーム"], ["salle de réunion", "会議室"], ["objectif", "目標"],
    ["résultat", "成果"], ["dossier", "文書"], ["plan", "計画"], ["opération", "運用"], ["point faible", "問題点"],
    ["amélioration", "改善"], ["coût", "費用"], ["chiffre d'affaires", "売上"], ["qualité", "品質"], ["calendrier", "予定表"],
    ["publication", "公開"], ["présentation", "発表"], ["matériel", "準備物"], ["décision", "決定"], ["réviser", "検討する"],
    ["signaler", "報告する"], ["demander", "依頼する"], ["modifier", "修正する"], ["publier", "公開する"], ["discuter", "議論する"],
    ["négocier", "交渉する"], ["soumettre", "提出する"], ["terminer", "完了する"], ["reporter", "延期する"], ["annuler", "キャンセルする"],
    ["gérer", "管理する"], ["ajouter", "追加する"], ["retirer", "削除する"], ["confirmé", "確認済み"], ["urgent", "急ぎの"],
    ["fournisseur", "仕入先"], ["livrable", "成果物"], ["échéance", "期限"], ["valider", "承認する"], ["relancer", "催促する"]
  ],
  travel: [
    ["départ", "出発"], ["arrivée", "到着"], ["entrée", "入口"], ["sortie", "出口"], ["guichet", "切符売り場"],
    ["quai", "ホーム"], ["métro", "地下鉄"], ["train", "列車"], ["avion", "飛行機"], ["siège", "座席"],
    ["couloir", "通路"], ["côté fenêtre", "窓側"], ["douane", "税関"], ["hébergement", "宿泊先"], ["réception", "フロント"],
    ["clé", "鍵"], ["serviette", "タオル"], ["douche", "シャワー"], ["climatisation", "エアコン"], ["chauffage", "暖房"],
    ["petit déjeuner", "朝食"], ["commande", "注文"], ["baguettes", "箸"], ["à emporter", "持ち帰り"], ["prix", "価格"],
    ["réduction", "割引"], ["espèces", "現金"], ["monnaie", "おつり"], ["souvenir", "お土産"], ["taille", "サイズ"],
    ["couleur", "色"], ["échange", "交換"], ["remboursement", "返金"], ["objet perdu", "忘れ物"], ["poste de police", "警察署"],
    ["urgences", "救急外来"], ["assurance", "保険"], ["confirmation de réservation", "予約確認"], ["itinéraire", "道案内"], ["site touristique", "観光地"],
    ["billet d'entrée", "入場券"], ["horaires d'ouverture", "営業時間"], ["fermer", "閉まる"], ["ouvrir", "開く"], ["marcher", "歩く"],
    ["changer de train", "乗り換える"], ["modifier la réservation", "予約を変更する"], ["direct", "直通"], ["près d'ici", "近く"], ["sûr", "安全な"]
  ]
};

const frenchPhraseBoost = {
  basic: [
    ["Vous avez du temps aujourd'hui ?", "今日時間はありますか"], ["Je suis un peu fatigué.", "少し疲れています"], ["Un verre d'eau, s'il vous plaît.", "水を一杯ください"],
    ["Je prends ceci.", "これをください"], ["On peut y aller doucement.", "ゆっくりで大丈夫です"], ["Je peux partir maintenant ?", "今行ってもいいですか"],
    ["Je peux m'asseoir ici ?", "ここに座ってもいいですか"], ["Regardez cette photo, s'il vous plaît.", "この写真を見てください"], ["Je m'appelle ○○.", "私の名前は○○です"],
    ["Je veux rentrer chez moi.", "家に帰りたいです"], ["J'attends un ami.", "友だちを待っています"], ["Il fait beau aujourd'hui.", "今日は天気がいいです"],
    ["Ce plat est délicieux.", "この料理はおいしいです"], ["Je voudrais boire un café.", "コーヒーを飲みたいです"], ["À la prochaine.", "また次に会いましょう"],
    ["Qu'est-ce que ça veut dire ?", "これはどういう意味ですか"], ["C'est bien ici ?", "ここで合っていますか"], ["Je peux téléphoner un instant ?", "少し電話してもいいですか"],
    ["On s'arrête ici pour aujourd'hui.", "今日はここまでにします"], ["Je pense pareil.", "私もそう思います"], ["Je ne suis pas encore prêt.", "まだ準備できていません"],
    ["Essayez d'abord.", "先にやってみてください"], ["Allons-y ensemble.", "一緒に行きましょう"], ["Faites attention.", "気をつけてください"], ["Dites-moi si c'est possible.", "可能なら教えてください"]
  ],
  practical: [
    ["Est-ce que j'ai bien compris ?", "私の理解で合っていますか"], ["Je vais le résumer encore une fois.", "もう一度整理します"], ["Cette expression est naturelle ?", "この表現は自然ですか"],
    ["J'ai besoin d'exemples plus simples.", "もっと簡単な例文が必要です"], ["Vérifiez ma phrase corrigée, s'il vous plaît.", "私が直した文を確認してください"], ["Comment utilise-t-on ce mot ?", "この単語をどう使いますか"],
    ["Je veux réviser ce que j'ai appris aujourd'hui.", "今日学んだ内容を復習したいです"], ["Je reverrai cela plus tard.", "これはあとでまた見ます"], ["Laissez-moi un peu de temps pour réfléchir.", "少し考えさせてください"],
    ["Dites-moi où je me suis trompé.", "どこを間違えたか教えてください"], ["Je veux connaître la raison, pas seulement la réponse.", "答えだけでなく理由も知りたいです"], ["Expliquez la différence entre ces deux expressions.", "二つの表現の違いを説明してください"],
    ["Je veux parler selon la situation.", "場面に合わせて話したいです"], ["Je vais sauvegarder mes progrès et continuer.", "記録を保存して続けます"], ["Je veux atteindre mon objectif d'aujourd'hui.", "今日の目標を達成したいです"],
    ["Rendez-le un peu plus naturel.", "もう少し自然にしてください"], ["Faites écouter la prononciation encore une fois.", "発音をもう一度聞かせてください"], ["Cette phrase est trop formelle.", "この文は硬すぎます"],
    ["Peut-on l'utiliser dans la conversation quotidienne ?", "日常会話で使えますか"], ["Je vais essayer de faire une phrase moi-même.", "自分で文を作ってみます"], ["Je veux retravailler les mots qui se ressemblent.", "紛らわしい単語をもう一度練習します"],
    ["Ce n'est pas urgent.", "これは急ぎではありません"], ["Je commence par le plus important.", "先に重要なものからやります"], ["Donnez-moi aussi une expression similaire.", "似た表現も教えてください"], ["Aujourd'hui, je veux travailler surtout l'écoute.", "今日は聞く練習を中心にします"]
  ],
  business: [
    ["Je vais vérifier l'ordre du jour.", "会議議題を確認します"], ["Les documents seront partagés avant la réunion.", "資料は会議前に共有します"], ["Vérifiez d'abord le responsable.", "担当者を先に確認してください"],
    ["Ce point doit être confirmé avec le client.", "この件は顧客確認が必要です"], ["Peut-on reporter la date limite d'un jour ?", "締切を1日延ばせますか"], ["Renvoyez le fichier dans sa dernière version.", "最新版のファイルで送り直してください"],
    ["Je vais résumer les décisions.", "決定事項を短く整理します"], ["Signalez-moi tout risque immédiatement.", "リスクがあればすぐ教えてください"], ["Nous avancerons dans la limite du budget.", "予算範囲内で進めます"],
    ["Il faut revoir les priorités.", "優先順位を見直す必要があります"], ["Cette formulation est trop forte pour le client.", "この表現は顧客には強すぎます"], ["Écrivez clairement la prochaine action.", "次の行動を明確に書いてください"],
    ["Après vérification, je demanderai l'approbation.", "確認後に承認依頼を出します"], ["Je donnerai une première réponse aujourd'hui.", "本日中に一次回答します"], ["Je rédigerai le compte rendu.", "議事録は私が作成します"],
    ["Je partagerai immédiatement tout problème.", "問題が起きたらすぐ共有します"], ["Nous en discuterons en interne.", "この部分は社内で議論します"], ["Il faut revérifier les conditions du contrat.", "契約条件を再確認する必要があります"],
    ["Je vais voir si le planning peut être ajusté.", "日程変更が可能か確認します"], ["La version finale sera envoyée demain matin.", "最終版は明日の午前に送ります"], ["Clarifions le périmètre de responsabilité.", "担当範囲を明確にしましょう"],
    ["J'ai listé les demandes.", "依頼事項をリストにしました"], ["Je garderai une trace par e-mail.", "この内容はメールに残します"], ["Nous reprendrons ce sujet à la prochaine réunion.", "次の会議で再度扱います"], ["Des frais supplémentaires peuvent s'appliquer.", "追加費用が発生する可能性があります"]
  ],
  travel: [
    ["Combien de temps faut-il pour aller à l'aéroport ?", "空港までどのくらいかかりますか"], ["Ce billet est valable aujourd'hui ?", "このチケットは今日使えますか"], ["Où est la gare la plus proche ?", "一番近い駅はどこですか"],
    ["Puis-je partir plus tard de la chambre ?", "チェックアウトを遅らせられますか"], ["Il n'y a pas d'eau chaude dans la chambre.", "部屋でお湯が出ません"], ["La climatisation ne marche pas.", "エアコンが動きません"],
    ["Puis-je avoir une chambre plus calme ?", "静かな部屋に変えられますか"], ["Allez à cette adresse, s'il vous plaît.", "この住所へ行ってください"], ["Peut-on prendre des photos ici ?", "ここで写真を撮ってもいいですか"],
    ["Quel plat recommandez-vous ?", "おすすめ料理は何ですか"], ["Sans viande, s'il vous plaît.", "肉なしでお願いします"], ["À emporter, s'il vous plaît.", "持ち帰りにしてください"],
    ["Peut-on payer séparément ?", "会計を別々にできますか"], ["Vous pouvez me rendre la monnaie ?", "おつりをもらえますか"], ["Je voudrais échanger cet article.", "この商品を交換したいです"],
    ["Puis-je avoir un autre reçu ?", "レシートをもう一度もらえますか"], ["J'ai perdu mon passeport.", "パスポートをなくしました"], ["J'ai besoin d'aide maintenant.", "今助けが必要です"],
    ["Je veux aller à l'hôpital le plus proche.", "近くの病院に行きたいです"], ["Où puis-je acheter des médicaments ?", "どこで薬を買えますか"], ["Je voudrais laisser mes bagages jusqu'au soir.", "荷物を夜まで預けたいです"],
    ["Cette rue est-elle sûre la nuit ?", "この道は夜でも安全ですか"], ["Où acheter les billets d'entrée ?", "入場券はどこで買いますか"], ["C'est ouvert aujourd'hui ?", "今日は営業していますか"], ["Puis-je payer sans espèces ?", "現金なしで支払えますか"]
  ]
};

const koreanWordBoost2 = {
  basic: [
    ["남자", "男性"], ["여자", "女性"], ["아기", "赤ちゃん"], ["형", "兄"], ["누나", "姉"],
    ["동생", "弟 / 妹"], ["시장", "市場"], ["공원", "公園"], ["거리", "通り / 距離"], ["우유", "牛乳"],
    ["계란", "卵"], ["노래", "歌"], ["운동", "運動"], ["읽다", "読む"], ["쓰다", "書く / 使う"]
  ],
  practical: [
    ["생활비", "生活費"], ["알림", "通知"], ["메모", "メモ"], ["앱 설정", "アプリ設定"], ["로그인", "ログイン"],
    ["저장", "保存"], ["삭제", "削除"], ["복습", "復習"], ["반복", "反復"], ["연결", "接続"],
    ["실패", "失敗"], ["성공", "成功"], ["추천", "おすすめ"], ["익숙하다", "慣れている"], ["확실하다", "確かだ"]
  ],
  business: [
    ["견적", "見積もり"], ["청구서", "請求書"], ["납품", "納品"], ["매뉴얼", "マニュアル"], ["권한", "権限"],
    ["보안", "セキュリティ"], ["재고", "在庫"], ["담당", "担当"], ["승인 요청", "承認依頼"], ["회신", "返信"],
    ["지연", "遅延"], ["합의", "合意"], ["공유 폴더", "共有フォルダ"], ["운영팀", "運用チーム"], ["검수", "検収"]
  ],
  travel: [
    ["탑승구", "搭乗口"], ["수하물", "預け荷物"], ["환전", "両替"], ["여권 번호", "パスポート番号"], ["숙박비", "宿泊費"],
    ["체크아웃", "チェックアウト"], ["온수", "お湯"], ["조용한 방", "静かな部屋"], ["분실", "紛失"], ["안내소", "案内所"],
    ["버스 정류장", "バス停"], ["약", "薬"], ["통역", "通訳"], ["영업 중", "営業中"], ["매진", "売り切れ"]
  ]
};

const koreanPhraseBoost2 = {
  basic: [
    ["오늘은 집에서 쉬고 싶어요.", "今日は家で休みたいです"], ["이 노래를 좋아해요.", "この歌が好きです"], ["공원에서 조금 걸었어요.", "公園で少し歩きました"],
    ["우유를 하나 사 주세요.", "牛乳を一つ買ってください"], ["천천히 읽어 볼게요.", "ゆっくり読んでみます"], ["제가 먼저 쓸게요.", "私が先に書きます"],
    ["동생이 아직 안 왔어요.", "弟 / 妹がまだ来ていません"], ["편의점이 어디에 있어요?", "コンビニはどこにありますか"], ["운동을 자주 해요.", "よく運動します"], ["이 거리는 조용해요.", "この通りは静かです"]
  ],
  practical: [
    ["알림을 꺼도 될까요?", "通知を切ってもいいですか"], ["메모로 남겨 둘게요.", "メモに残しておきます"], ["로그인이 잘 안 돼요.", "ログインがうまくできません"],
    ["파일을 다시 저장해 주세요.", "ファイルをもう一度保存してください"], ["삭제하기 전에 확인해 주세요.", "削除する前に確認してください"], ["복습할 내용을 표시해 주세요.", "復習する内容を表示してください"],
    ["같은 문장을 반복해서 들어요.", "同じ文を繰り返し聞きます"], ["인터넷 연결이 끊겼어요.", "インターネット接続が切れました"], ["성공한 방법을 알려 주세요.", "うまくいった方法を教えてください"], ["아직 익숙하지 않아요.", "まだ慣れていません"]
  ],
  business: [
    ["견적서를 다시 보내 주세요.", "見積書を送り直してください"], ["청구서 발행일을 확인하겠습니다.", "請求書の発行日を確認します"], ["납품 일정을 조정해야 합니다.", "納品日程を調整する必要があります"],
    ["권한 설정을 확인해 주세요.", "権限設定を確認してください"], ["보안상 공유할 수 없습니다.", "セキュリティ上、共有できません"], ["재고가 부족합니다.", "在庫が不足しています"],
    ["승인 요청을 올리겠습니다.", "承認依頼を出します"], ["회신이 아직 없습니다.", "返信がまだありません"], ["일정이 지연되고 있습니다.", "日程が遅れています"], ["운영팀에 확인하겠습니다.", "運用チームに確認します"]
  ],
  travel: [
    ["탑승구가 변경되었습니다.", "搭乗口が変更されました"], ["수하물을 어디서 찾나요?", "預け荷物はどこで受け取りますか"], ["근처에서 환전할 수 있나요?", "近くで両替できますか"],
    ["체크아웃 시간을 알고 싶어요.", "チェックアウト時間を知りたいです"], ["방에 온수가 안 나와요.", "部屋でお湯が出ません"], ["조용한 방으로 바꿀 수 있나요?", "静かな部屋に変えられますか"],
    ["분실물을 신고하고 싶어요.", "忘れ物を届け出たいです"], ["버스 정류장은 어디인가요?", "バス停はどこですか"], ["약을 살 수 있는 곳이 있나요?", "薬を買える場所はありますか"], ["오늘 표는 매진인가요?", "今日のチケットは売り切れですか"],
    ["짐 보관 시간이 몇 시까지예요?", "荷物預かりは何時までですか"], ["택시비는 대략 얼마인가요?", "タクシー代はだいたいいくらですか"]
  ]
};

const chineseWordBoost2 = {
  basic: [
    ["男人", "男性"], ["女人", "女性"], ["孩子", "子ども"], ["宝宝", "赤ちゃん"], ["哥哥", "兄"],
    ["姐姐", "姉"], ["弟弟", "弟"], ["妹妹", "妹"], ["市场", "市場"], ["公园", "公園"], ["衣服", "服"],
    ["街道", "通り"], ["牛奶", "牛乳"], ["鸡蛋", "卵"], ["唱歌", "歌う"], ["写字", "字を書く"]
  ],
  practical: [
    ["生活费", "生活費"], ["通知", "通知"], ["笔记", "メモ"], ["设置", "設定"], ["登录", "ログイン"],
    ["备份", "バックアップ"], ["清理", "片付ける / 整理する"], ["回顾", "振り返る"], ["重复", "繰り返す"], ["连接", "接続"],
    ["失败", "失敗"], ["成功", "成功"], ["建议", "提案 / アドバイス"], ["习惯", "習慣"], ["清楚", "はっきりした"],
    ["排序", "並べ替える"]
  ],
  business: [
    ["报价", "見積もり"], ["发票", "請求書 / 領収書"], ["交付", "納品"], ["手册", "マニュアル"], ["权限", "権限"],
    ["安全", "セキュリティ"], ["库存", "在庫"], ["负责", "担当する"], ["审批申请", "承認依頼"], ["回信", "返信"],
    ["延迟", "遅延"], ["共识", "合意"], ["共享文件夹", "共有フォルダ"], ["运营团队", "運用チーム"], ["验收", "検収"]
  ],
  travel: [
    ["登机口", "搭乗口"], ["托运行李", "預け荷物"], ["换钱", "両替する"], ["护照号码", "パスポート番号"], ["住宿费", "宿泊費"],
    ["退房", "チェックアウト"], ["热水", "お湯"], ["安静的房间", "静かな部屋"], ["丢失", "紛失"], ["问讯处", "案内所"],
    ["公交车站", "バス停"], ["药", "薬"], ["翻译", "通訳 / 翻訳"], ["营业中", "営業中"], ["卖完", "売り切れ"]
  ]
};

const chinesePhraseBoost2 = {
  basic: [
    ["今天我想在家休息。", "今日は家で休みたいです"], ["我喜欢这首歌。", "この歌が好きです"], ["我在公园走了一会儿。", "公園で少し歩きました"],
    ["请买一瓶牛奶。", "牛乳を1本買ってください"], ["我慢慢读一遍。", "ゆっくり一度読みます"], ["我先写。", "私が先に書きます"],
    ["孩子还没回来。", "子どもがまだ帰っていません"], ["便利店在哪里？", "コンビニはどこですか"], ["我经常运动。", "よく運動します"], ["这条街很安静。", "この通りは静かです"]
  ],
  practical: [
    ["可以关掉通知吗？", "通知を切ってもいいですか"], ["我会记在笔记里。", "メモに残します"], ["我登录不了。", "ログインできません"],
    ["请再保存一次文件。", "ファイルをもう一度保存してください"], ["删除前请确认。", "削除する前に確認してください"], ["请显示要复习的内容。", "復習する内容を表示してください"],
    ["我会反复听同一句话。", "同じ文を繰り返し聞きます"], ["网络连接断了。", "ネット接続が切れました"], ["请告诉我成功的方法。", "うまくいった方法を教えてください"], ["我还不习惯。", "まだ慣れていません"]
  ],
  business: [
    ["请重新发送报价单。", "見積書を送り直してください"], ["我确认一下发票日期。", "請求書の日付を確認します"], ["需要调整交付日期。", "納品日を調整する必要があります"],
    ["请确认权限设置。", "権限設定を確認してください"], ["出于安全原因不能共享。", "セキュリティ上、共有できません"], ["库存不够。", "在庫が足りません"],
    ["我会提交审批申请。", "承認依頼を出します"], ["还没有收到回信。", "返信がまだありません"], ["进度有延迟。", "進捗が遅れています"], ["我会和运营团队确认。", "運用チームに確認します"]
  ],
  travel: [
    ["登机口变了。", "搭乗口が変わりました"], ["托运行李在哪里取？", "預け荷物はどこで受け取りますか"], ["附近可以换钱吗？", "近くで両替できますか"],
    ["我想知道退房时间。", "チェックアウト時間を知りたいです"], ["房间没有热水。", "部屋でお湯が出ません"], ["可以换到安静的房间吗？", "静かな部屋に変えられますか"],
    ["我想申报丢失物品。", "忘れ物を届け出たいです"], ["公交车站在哪里？", "バス停はどこですか"], ["附近有医院吗？", "近くに病院はありますか"], ["今天的票卖完了吗？", "今日のチケットは売り切れですか"],
    ["出租车费大概多少钱？", "タクシー代はだいたいいくらですか"]
  ]
};

const frenchWordBoost2 = {
  basic: [
    ["homme", "男性"], ["femme", "女性"], ["bébé", "赤ちゃん"], ["frère", "兄 / 弟"], ["sœur", "姉 / 妹"],
    ["marché", "市場"], ["parc", "公園"], ["rue", "通り"], ["lait", "牛乳"], ["œuf", "卵"],
    ["chanson", "歌"], ["sport", "運動"], ["lire", "読む"], ["utiliser", "使う"], ["acheter", "買う"]
  ],
  practical: [
    ["dépenses", "生活費"], ["notification", "通知"], ["note", "メモ"], ["réglage", "設定"], ["connexion", "ログイン / 接続"],
    ["enregistrer", "保存する"], ["effacer", "消す"], ["révision", "復習"], ["répétition", "反復"], ["échec", "失敗"],
    ["réussite", "成功"], ["conseil", "助言 / おすすめ"], ["habitué", "慣れている"], ["clair", "はっきりした"], ["rapide", "速い"]
  ],
  business: [
    ["devis", "見積もり"], ["facture", "請求書"], ["livraison", "納品 / 配送"], ["manuel", "マニュアル"], ["autorisation", "権限"],
    ["sécurité", "セキュリティ"], ["stock", "在庫"], ["demande d'approbation", "承認依頼"], ["retard", "遅延"], ["accord", "合意"],
    ["dossier partagé", "共有フォルダ"], ["équipe opérationnelle", "運用チーム"], ["recette", "検収"], ["prioritaire", "優先の"], ["mise à jour", "更新"]
  ],
  travel: [
    ["porte d'embarquement", "搭乗口"], ["bagage enregistré", "預け荷物"], ["change", "両替"], ["numéro de passeport", "パスポート番号"], ["frais d'hébergement", "宿泊費"],
    ["départ de l'hôtel", "チェックアウト"], ["eau chaude", "お湯"], ["chambre calme", "静かな部屋"], ["perte", "紛失"], ["point d'information", "案内所"],
    ["arrêt de bus", "バス停"], ["médicament", "薬"], ["interprète", "通訳"], ["ouvert", "営業中"], ["complet", "満席 / 売り切れ"]
  ]
};

const frenchPhraseBoost2 = {
  basic: [
    ["Aujourd'hui, je veux me reposer à la maison.", "今日は家で休みたいです"], ["J'aime cette chanson.", "この歌が好きです"], ["Je me suis promené au parc.", "公園で少し歩きました"],
    ["Achetez du lait, s'il vous plaît.", "牛乳を買ってください"], ["Je vais lire lentement.", "ゆっくり読みます"], ["J'écris d'abord.", "私が先に書きます"],
    ["Mon frère n'est pas encore arrivé.", "兄 / 弟がまだ来ていません"], ["Où est la supérette ?", "コンビニはどこですか"], ["Je fais souvent du sport.", "よく運動します"], ["Cette rue est calme.", "この通りは静かです"]
  ],
  practical: [
    ["Je peux désactiver les notifications ?", "通知を切ってもいいですか"], ["Je vais le noter.", "メモしておきます"], ["Je n'arrive pas à me connecter.", "ログインできません"],
    ["Enregistrez encore le fichier, s'il vous plaît.", "ファイルをもう一度保存してください"], ["Vérifiez avant de supprimer.", "削除する前に確認してください"], ["Affichez ce que je dois réviser.", "復習する内容を表示してください"],
    ["J'écoute la même phrase plusieurs fois.", "同じ文を何度も聞きます"], ["La connexion Internet est coupée.", "ネット接続が切れました"], ["Dites-moi ce qui a bien marché.", "うまくいった方法を教えてください"], ["Je ne suis pas encore habitué.", "まだ慣れていません"]
  ],
  business: [
    ["Renvoyez le devis, s'il vous plaît.", "見積書を送り直してください"], ["Je vais vérifier la date de la facture.", "請求書の日付を確認します"], ["Il faut ajuster la date de livraison.", "納品日を調整する必要があります"],
    ["Vérifiez les autorisations.", "権限を確認してください"], ["On ne peut pas partager cela pour des raisons de sécurité.", "セキュリティ上、共有できません"], ["Le stock est insuffisant.", "在庫が足りません"],
    ["Je vais faire une demande d'approbation.", "承認依頼を出します"], ["Je n'ai pas encore reçu de réponse.", "返信をまだ受け取っていません"], ["Le planning prend du retard.", "予定が遅れています"], ["Je vais vérifier avec l'équipe opérationnelle.", "運用チームに確認します"]
  ],
  travel: [
    ["La porte d'embarquement a changé.", "搭乗口が変わりました"], ["Où récupère-t-on les bagages enregistrés ?", "預け荷物はどこで受け取りますか"], ["Peut-on faire du change près d'ici ?", "近くで両替できますか"],
    ["Je voudrais connaître l'heure de départ de l'hôtel.", "チェックアウト時間を知りたいです"], ["Il n'y a pas d'eau chaude dans la chambre.", "部屋でお湯が出ません"], ["Puis-je changer pour une chambre plus calme ?", "もっと静かな部屋に変えられますか"],
    ["Je voudrais déclarer un objet perdu.", "忘れ物を届け出たいです"], ["Où est l'arrêt de bus ?", "バス停はどこですか"], ["Y a-t-il un hôpital près d'ici ?", "近くに病院はありますか"], ["Les billets d'aujourd'hui sont complets ?", "今日のチケットは売り切れですか"],
    ["Combien coûte environ le taxi ?", "タクシー代はだいたいいくらですか"]
  ]
};

const koreanWordBoost3 = {
  basic: [
    ["안녕하세요", "こんにちは"], ["감사합니다", "ありがとうございます"], ["죄송합니다", "すみません"], ["괜찮아요", "大丈夫です"], ["진짜", "本当に"],
    ["다시", "もう一度 / 再び"], ["조금", "少し"], ["많이", "たくさん"], ["빨리", "早く"], ["천천히", "ゆっくり"],
    ["여기", "ここ"], ["저기", "あそこ"], ["오른쪽", "右側"], ["왼쪽", "左側"], ["근처", "近く"]
  ],
  practical: [
    ["모르겠습니다", "分かりません"], ["괜찮습니다", "大丈夫です"], ["부탁합니다", "お願いします"], ["필요해요", "必要です"], ["가능해요", "できます / 可能です"],
    ["어려워요", "難しいです"], ["쉬워요", "簡単です"], ["맞아요", "合っています"], ["틀려요", "間違っています"], ["천천히 말하다", "ゆっくり話す"]
  ],
  business: [
    ["일정 조정", "日程調整"], ["확인 요청", "確認依頼"], ["회의 자료", "会議資料"], ["진행 상황", "進捗状況"], ["마감일", "締切日"],
    ["담당 범위", "担当範囲"], ["추가 비용", "追加費用"], ["내부 검토", "社内確認 / 内部検討"], ["최종 확인", "最終確認"], ["우선 처리", "優先対応"]
  ],
  travel: [
    ["얼마예요?", "いくらですか"], ["어디예요?", "どこですか"], ["도와주세요", "助けてください"], ["예약했어요", "予約しました"], ["길을 잃었어요", "道に迷いました"],
    ["화장실", "トイレ"], ["편의점", "コンビニ"], ["영수증 주세요", "レシートをください"], ["카드 돼요?", "カードは使えますか"], ["물 주세요", "水をください"]
  ]
};

const koreanPhraseBoost3 = {
  basic: [
    ["안녕하세요. 잘 지냈어요?", "こんにちは。元気でしたか"], ["오랜만이에요.", "お久しぶりです"], ["보고 싶었어요.", "会いたかったです"],
    ["잘 부탁합니다.", "よろしくお願いします"], ["진짜 고마워요.", "本当にありがとうございます"], ["다음에 또 봐요.", "また今度会いましょう"],
    ["괜찮아요. 천천히 하세요.", "大丈夫です。ゆっくりしてください"], ["죄송합니다. 잘 모르겠습니다.", "すみません。よく分かりません"],
    ["다시 한번 말해 주세요.", "もう一度言ってください"], ["천천히 말해 주세요.", "ゆっくり話してください"],
    ["이건 제 물건이에요.", "これは私の物です"], ["저는 지금 시간이 없어요.", "今は時間がありません"], ["조금만 기다려 주세요.", "少しだけ待ってください"],
    ["여기서 기다릴게요.", "ここで待っています"], ["저쪽으로 가면 돼요?", "あちらへ行けばいいですか"], ["오늘은 시간이 괜찮아요.", "今日は時間があります"],
    ["이 사람을 알고 있어요.", "この人を知っています"], ["먼저 물을 마실게요.", "先に水を飲みます"], ["문을 닫아 주세요.", "ドアを閉めてください"], ["창문을 열어도 돼요?", "窓を開けてもいいですか"]
  ],
  practical: [
    ["이 표현은 일상에서 자주 써요.", "この表現は日常でよく使います"], ["제가 이해한 내용을 다시 말해 볼게요.", "理解した内容を言い直してみます"],
    ["비슷한 말을 하나 더 알려 주세요.", "似た言い方をもう一つ教えてください"], ["어떤 상황에서 쓰면 좋아요?", "どんな場面で使うといいですか"],
    ["틀린 부분만 짧게 알려 주세요.", "間違った部分だけ短く教えてください"], ["오늘 배운 표현을 바로 써 볼게요.", "今日覚えた表現をすぐ使ってみます"]
  ],
  business: [
    ["확인 후 바로 회신드리겠습니다.", "確認後すぐに返信します"], ["일정을 다시 조정해 보겠습니다.", "日程を再調整してみます"],
    ["진행 상황을 공유해 주세요.", "進捗状況を共有してください"], ["담당 범위를 먼저 정리하겠습니다.", "担当範囲を先に整理します"],
    ["추가 비용이 있는지 확인하겠습니다.", "追加費用があるか確認します"], ["최종 확인 후 진행하겠습니다.", "最終確認後に進めます"]
  ],
  travel: [
    ["화장실이 어디에 있어요?", "トイレはどこにありますか"], ["이 근처에 편의점이 있나요?", "この近くにコンビニはありますか"],
    ["카드로 결제할 수 있나요?", "カードで支払えますか"], ["영수증을 주세요.", "レシートをください"],
    ["예약한 사람입니다.", "予約している者です"], ["길을 잃었어요. 도와주세요.", "道に迷いました。助けてください"],
    ["가방을 잃어버렸어요.", "かばんをなくしました"], ["이 주소까지 가 주세요.", "この住所まで行ってください"], ["가장 가까운 역으로 가고 싶어요.", "一番近い駅へ行きたいです"],
    ["체크인하고 싶어요.", "チェックインしたいです"], ["방을 확인해 주세요.", "部屋を確認してください"], ["수건을 더 받을 수 있나요?", "タオルを追加でもらえますか"],
    ["아침 식사는 몇 시부터예요?", "朝食は何時からですか"], ["예약을 취소하고 싶어요.", "予約をキャンセルしたいです"], ["이 길로 가면 되나요?", "この道で合っていますか"],
    ["표를 한 장 사고 싶어요.", "チケットを1枚買いたいです"], ["환불할 수 있나요?", "返金できますか"], ["조금 더 큰 사이즈가 있나요?", "もう少し大きいサイズはありますか"],
    ["이 근처는 밤에 안전한가요?", "この辺りは夜安全ですか"], ["응급실에 가야 해요.", "救急外来に行く必要があります"], ["보험 서류가 필요해요.", "保険の書類が必要です"]
  ]
};

const chineseWordBoost3 = {
  basic: [
    ["你好", "こんにちは"], ["谢谢", "ありがとう"], ["对不起", "すみません"], ["没关系", "大丈夫"], ["真的", "本当に"],
    ["再", "もう一度 / 再び"], ["一点儿", "少し"], ["很多", "たくさん"], ["快", "速い / 早く"], ["慢", "遅い / ゆっくり"],
    ["这里", "ここ"], ["那里", "そこ / あそこ"], ["右边", "右側"], ["左边", "左側"], ["附近", "近く"]
  ],
  practical: [
    ["不知道", "分からない"], ["没问题", "問題ありません"], ["麻烦你", "お願いします / お手数ですが"], ["需要", "必要だ"], ["可以", "できる / よい"],
    ["不可以", "できない / だめ"], ["正确", "正しい"], ["不对", "違う"], ["清楚", "はっきりしている"], ["慢慢说", "ゆっくり話す"]
  ],
  business: [
    ["日程调整", "日程調整"], ["确认请求", "確認依頼"], ["会议资料", "会議資料"], ["进展情况", "進捗状況"], ["截止日期", "締切日"],
    ["负责范围", "担当範囲"], ["追加费用", "追加費用"], ["内部确认", "社内確認"], ["最终确认", "最終確認"], ["优先处理", "優先対応"]
  ],
  travel: [
    ["多少钱?", "いくらですか"], ["在哪里?", "どこですか"], ["请帮忙", "助けてください"], ["我预订了", "予約しました"], ["我迷路了", "道に迷いました"],
    ["洗手间", "トイレ"], ["便利店", "コンビニ"], ["请给我收据", "レシートをください"], ["可以刷卡吗?", "カードは使えますか"], ["请给我水", "水をください"]
  ]
};

const chinesePhraseBoost3 = {
  basic: [
    ["你好，最近怎么样？", "こんにちは、最近どうですか"], ["好久不见。", "お久しぶりです"], ["我很想见你。", "会いたかったです"],
    ["请多关照。", "よろしくお願いします"], ["真的谢谢你。", "本当にありがとうございます"], ["下次再见。", "また今度会いましょう"],
    ["没关系，慢慢来。", "大丈夫です。ゆっくりでいいです"], ["对不起，我不太明白。", "すみません。よく分かりません"],
    ["请再说一遍。", "もう一度言ってください"], ["请慢一点说。", "ゆっくり話してください"],
    ["这是我的东西。", "これは私の物です"], ["我现在没有时间。", "今は時間がありません"], ["请等我一下。", "少し待ってください"],
    ["我在这里等。", "ここで待っています"], ["往那边走可以吗？", "あちらへ行けばいいですか"], ["今天我有时间。", "今日は時間があります"],
    ["我认识这个人。", "この人を知っています"], ["我先喝水。", "先に水を飲みます"], ["请关门。", "ドアを閉めてください"], ["可以开窗吗？", "窓を開けてもいいですか"]
  ],
  practical: [
    ["这个说法日常生活中常用。", "この言い方は日常生活でよく使います"], ["我把理解的内容再说一遍。", "理解した内容をもう一度言います"],
    ["请再告诉我一个类似的说法。", "似た言い方をもう一つ教えてください"], ["这个在什么情况下用比较好？", "これはどんな場面で使うといいですか"],
    ["请只告诉我错的地方。", "間違っているところだけ教えてください"], ["我马上用今天学的表达。", "今日覚えた表現をすぐ使ってみます"]
  ],
  business: [
    ["确认后我会马上回复。", "確認後すぐに返信します"], ["我会重新调整日程。", "日程を再調整します"],
    ["请共享进展情况。", "進捗状況を共有してください"], ["我先整理负责范围。", "担当範囲を先に整理します"],
    ["我确认一下有没有追加费用。", "追加費用があるか確認します"], ["最终确认后再推进。", "最終確認後に進めます"]
  ],
  travel: [
    ["洗手间在哪里？", "トイレはどこにありますか"], ["这附近有便利店吗？", "この近くにコンビニはありますか"],
    ["可以刷卡吗？", "カードで支払えますか"], ["请给我收据。", "レシートをください"],
    ["我有预订。", "予約しています"], ["我迷路了，请帮帮我。", "道に迷いました。助けてください"],
    ["我的包丢了。", "かばんをなくしました"], ["请去这个地址。", "この住所まで行ってください"], ["我想去最近的车站。", "一番近い駅へ行きたいです"],
    ["我想办理入住。", "チェックインしたいです"], ["请确认一下房间。", "部屋を確認してください"], ["可以多给我一条毛巾吗？", "タオルを追加でもらえますか"],
    ["早餐几点开始？", "朝食は何時からですか"], ["我想取消预订。", "予約をキャンセルしたいです"], ["走这条路对吗？", "この道で合っていますか"],
    ["我想买一张票。", "チケットを1枚買いたいです"], ["可以退款吗？", "返金できますか"], ["有大一点的尺码吗？", "もう少し大きいサイズはありますか"],
    ["这附近晚上安全吗？", "この辺りは夜安全ですか"], ["我需要去急诊。", "救急外来に行く必要があります"], ["我需要保险文件。", "保険の書類が必要です"],
    ["请帮我叫出租车。", "タクシーを呼んでください"], ["这班车到机场吗？", "このバスは空港に行きますか"], ["可以把行李寄存在这里吗？", "ここで荷物を預けられますか"],
    ["请不要放香菜。", "パクチーを入れないでください"], ["我想换到靠窗的座位。", "窓側の席に変えたいです"]
  ]
};

const frenchWordBoost3 = {
  basic: [
    ["bonjour", "こんにちは"], ["merci", "ありがとう"], ["désolé", "すみません"], ["d'accord", "分かりました / 了解"], ["vraiment", "本当に"],
    ["encore", "もう一度 / まだ"], ["un peu", "少し"], ["beaucoup", "たくさん"], ["vite", "速く"], ["lentement", "ゆっくり"],
    ["ici", "ここ"], ["là-bas", "あそこ"], ["à droite", "右へ / 右側"], ["à gauche", "左へ / 左側"], ["près d'ici", "この近く"]
  ],
  practical: [
    ["je ne sais pas", "分かりません"], ["pas de problème", "問題ありません"], ["s'il vous plaît", "お願いします"], ["j'ai besoin de", "〜が必要です"], ["c'est possible", "可能です"],
    ["ce n'est pas possible", "できません"], ["correct", "正しい"], ["faux", "間違った"], ["clair", "はっきりした"], ["parlez lentement", "ゆっくり話してください"]
  ],
  business: [
    ["ajustement du planning", "日程調整"], ["demande de vérification", "確認依頼"], ["documents de réunion", "会議資料"], ["état d'avancement", "進捗状況"], ["date limite", "締切日"],
    ["périmètre de responsabilité", "担当範囲"], ["frais supplémentaires", "追加費用"], ["vérification interne", "社内確認"], ["validation finale", "最終確認"], ["traitement prioritaire", "優先対応"]
  ],
  travel: [
    ["combien ça coûte ?", "いくらですか"], ["où est-ce ?", "どこですか"], ["aidez-moi", "助けてください"], ["j'ai réservé", "予約しました"], ["je suis perdu", "道に迷いました"],
    ["toilettes", "トイレ"], ["supérette", "コンビニ"], ["un reçu, s'il vous plaît", "レシートをください"], ["je peux payer par carte ?", "カードで払えますか"], ["de l'eau, s'il vous plaît", "水をください"]
  ]
};

const koreanWordBoost4 = {
  basic: [
    ["하루", "一日"], ["주", "週"], ["달", "月"], ["해", "年 / 太陽"], ["돈", "お金"],
    ["손", "手"], ["발", "足"], ["입", "口"], ["귀", "耳"], ["코", "鼻"],
    ["머리", "頭"], ["고기", "肉"], ["생선", "魚"], ["과일", "果物"], ["야채", "野菜"],
    ["모자", "帽子"], ["색", "色"], ["빨강", "赤"], ["파랑", "青"], ["초록", "緑"],
    ["노랑", "黄色"], ["검정", "黒"], ["하양", "白"], ["곳", "場所"]
  ],
  practical: [
    ["감정", "感情"], ["의견", "意見"], ["목표", "目標"], ["계획", "計画"], ["결과", "結果"],
    ["변화", "変化"], ["상태", "状態"], ["순서", "順番"], ["규칙", "ルール"], ["기회", "機会"],
    ["경험", "経験"], ["관계", "関係"], ["책임", "責任"], ["실력", "実力"], ["기본", "基本"],
    ["정확하다", "正確だ"], ["충분하다", "十分だ"], ["부족하다", "足りない"], ["편하다", "楽だ / 便利だ"], ["불편하다", "不便だ"]
  ],
  business: [
    ["업무량", "業務量"], ["매출액", "売上額"], ["손익", "損益"], ["성과물", "成果物"], ["기한", "期限"],
    ["초안", "下書き"], ["확정안", "確定案"], ["계약서", "契約書"], ["제안서", "提案書"], ["견적서", "見積書"],
    ["청구", "請求"], ["지급", "支払い"], ["검토자", "確認者"], ["회의록", "議事録"], ["요구사항", "要件"],
    ["우려", "懸念"], ["대안", "代案"], ["효율", "効率"], ["전략", "戦略"], ["지표", "指標"]
  ],
  travel: [
    ["항공권", "航空券"], ["출국", "出国"], ["입국", "入国"], ["환승", "乗り換え"], ["숙박", "宿泊"],
    ["객실", "客室"], ["열쇠", "鍵"], ["수건", "タオル"], ["샤워", "シャワー"], ["조식", "朝食"],
    ["저녁식사", "夕食"], ["할인", "割引"], ["계산대", "レジ"], ["진통제", "痛み止め"], ["증상", "症状"],
    ["열", "熱"], ["두통", "頭痛"], ["상처", "けが"], ["위치", "場所"], ["방향", "方向"]
  ]
};

const chineseWordBoost4 = {
  basic: [
    ["一天", "一日"], ["星期", "週"], ["月份", "月"], ["年份", "年"], ["钱", "お金"],
    ["手", "手"], ["脚", "足"], ["眼睛", "目"], ["嘴", "口"], ["耳朵", "耳"],
    ["鼻子", "鼻"], ["头", "頭"], ["肉", "肉"], ["鱼", "魚"], ["水果", "果物"],
    ["蔬菜", "野菜"], ["帽子", "帽子"], ["颜色", "色"], ["红色", "赤"], ["蓝色", "青"],
    ["绿色", "緑"], ["黄色", "黄色"], ["黑色", "黒"], ["白色", "白"], ["地方", "場所"]
  ],
  practical: [
    ["情绪", "感情"], ["意见", "意見"], ["目标", "目標"], ["计划", "計画"], ["结果", "結果"],
    ["变化", "変化"], ["状态", "状態"], ["顺序", "順番"], ["规则", "ルール"], ["机会", "機会"],
    ["经验", "経験"], ["关系", "関係"], ["责任", "責任"], ["能力", "能力"], ["基础", "基本"],
    ["准确", "正確だ"], ["足够", "十分だ"], ["不足", "足りない"], ["方便", "便利だ"], ["麻烦", "面倒だ"]
  ],
  business: [
    ["工作量", "業務量"], ["营业额", "売上額"], ["盈亏", "損益"], ["成果", "成果"], ["期限", "期限"],
    ["草稿", "下書き"], ["确定稿", "確定版"], ["合同书", "契約書"], ["提案书", "提案書"], ["报价单", "見積書"],
    ["请款", "請求"], ["支付", "支払い"], ["审核人", "確認者"], ["会议记录", "議事録"], ["需求", "要件"],
    ["顾虑", "懸念"], ["替代方案", "代案"], ["效率", "効率"], ["战略", "戦略"], ["指标", "指標"]
  ],
  travel: [
    ["机票", "航空券"], ["出境", "出国"], ["入境", "入国"], ["转机", "乗り換え"], ["住宿", "宿泊"],
    ["客房", "客室"], ["前台", "フロント"], ["钥匙", "鍵"], ["毛巾", "タオル"], ["淋浴", "シャワー"],
    ["早餐", "朝食"], ["晚餐", "夕食"], ["收银台", "レジ"], ["止痛药", "痛み止め"], ["症状", "症状"],
    ["发烧", "熱がある"], ["头痛", "頭痛"], ["受伤", "けが"], ["位置", "場所"], ["方向", "方向"]
  ]
};

const frenchWordBoost4 = {
  basic: [
    ["jour", "一日 / 日"], ["semaine", "週"], ["mois", "月"], ["année", "年"], ["argent", "お金"],
    ["main", "手"], ["pied", "足"], ["œil", "目"], ["bouche", "口"], ["oreille", "耳"],
    ["nez", "鼻"], ["tête", "頭"], ["viande", "肉"], ["poisson", "魚"], ["fruit", "果物"],
    ["légume", "野菜"], ["chapeau", "帽子"], ["couleur", "色"], ["rouge", "赤"], ["bleu", "青"],
    ["vert", "緑"], ["jaune", "黄色"], ["noir", "黒"], ["blanc", "白"], ["endroit", "場所"]
  ],
  practical: [
    ["émotion", "感情"], ["avis", "意見"], ["objectif", "目標"], ["résultat", "結果"], ["changement", "変化"],
    ["état", "状態"], ["ordre", "順番"], ["règle", "ルール"], ["occasion", "機会"], ["expérience", "経験"],
    ["relation", "関係"], ["responsabilité", "責任"], ["capacité", "能力"], ["base", "基本"], ["exact", "正確な"],
    ["suffisant", "十分な"], ["insuffisant", "足りない"], ["pratique", "便利な"], ["gênant", "不便な"], ["essentiel", "大切な"]
  ],
  business: [
    ["volume", "業務量"], ["rentabilité", "収益性"], ["délai", "期限"], ["brouillon", "下書き"], ["version", "版"],
    ["paiement", "支払い"], ["validateur", "確認者"], ["compte-rendu", "議事録"], ["besoin", "要件"], ["souci", "懸念"],
    ["solution", "解決策"], ["efficacité", "効率"], ["stratégie", "戦略"], ["indicateur", "指標"], ["processus", "手順"],
    ["ressource", "リソース"], ["charge", "負荷"], ["marge", "利益幅"], ["signature", "署名"], ["négociation", "交渉"]
  ],
  travel: [
    ["vol", "航空便"], ["correspondance", "乗り換え"], ["logement", "宿泊"], ["clé", "鍵"], ["serviette", "タオル"],
    ["douche", "シャワー"], ["petit-déjeuner", "朝食"], ["dîner", "夕食"], ["caisse", "レジ"], ["antalgique", "痛み止め"],
    ["symptôme", "症状"], ["fièvre", "熱"], ["migraine", "頭痛"], ["blessure", "けが"], ["emplacement", "場所"],
    ["direction", "方向"], ["bagagerie", "荷物預かり"], ["embarquement", "搭乗"], ["annulation", "キャンセル"], ["retard", "遅延"]
  ]
};

const frenchPhraseBoost3 = {
  basic: [
    ["Bonjour, comment allez-vous ?", "こんにちは、お元気ですか"], ["Ça fait longtemps.", "お久しぶりです"], ["J'avais envie de vous voir.", "会いたかったです"],
    ["Enchanté, je compte sur vous.", "はじめまして、よろしくお願いします"], ["Merci beaucoup, vraiment.", "本当にありがとうございます"], ["À la prochaine fois.", "また今度会いましょう"],
    ["Ce n'est pas grave, prenez votre temps.", "大丈夫です。ゆっくりでいいです"], ["Désolé, je ne comprends pas bien.", "すみません。よく分かりません"],
    ["Répétez encore une fois, s'il vous plaît.", "もう一度言ってください"], ["Parlez plus lentement, s'il vous plaît.", "もっとゆっくり話してください"],
    ["C'est à moi.", "これは私の物です"], ["Je n'ai pas le temps maintenant.", "今は時間がありません"], ["Attendez un instant, s'il vous plaît.", "少し待ってください"],
    ["Je vous attends ici.", "ここで待っています"], ["Je dois aller par là ?", "あちらへ行けばいいですか"], ["J'ai du temps aujourd'hui.", "今日は時間があります"],
    ["Je connais cette personne.", "この人を知っています"], ["Je bois d'abord de l'eau.", "先に水を飲みます"], ["Fermez la porte, s'il vous plaît.", "ドアを閉めてください"], ["Je peux ouvrir la fenêtre ?", "窓を開けてもいいですか"]
  ],
  practical: [
    ["Cette tournure s'utilise souvent au quotidien.", "この言い方は日常でよく使います"], ["Je vais reformuler ce que j'ai compris.", "理解した内容を言い直します"],
    ["Donnez-moi une autre expression proche.", "似た表現をもう一つ教えてください"], ["Dans quelle situation peut-on l'utiliser ?", "どんな場面で使えますか"],
    ["Indiquez seulement la partie incorrecte.", "間違っている部分だけ教えてください"], ["Je vais utiliser tout de suite l'expression d'aujourd'hui.", "今日覚えた表現をすぐ使ってみます"]
  ],
  business: [
    ["Je répondrai dès que ce sera vérifié.", "確認でき次第返信します"], ["Je vais réajuster le planning.", "日程を再調整します"],
    ["Partagez l'état d'avancement, s'il vous plaît.", "進捗状況を共有してください"], ["Je vais clarifier le périmètre de responsabilité.", "担当範囲を明確にします"],
    ["Je vérifie s'il y a des frais supplémentaires.", "追加費用があるか確認します"], ["Nous avancerons après la validation finale.", "最終確認後に進めます"]
  ],
  travel: [
    ["Où sont les toilettes ?", "トイレはどこにありますか"], ["Y a-t-il une supérette près d'ici ?", "この近くにコンビニはありますか"],
    ["Je peux payer par carte ?", "カードで支払えますか"], ["Un reçu, s'il vous plaît.", "レシートをください"],
    ["J'ai une réservation.", "予約しています"], ["Je suis perdu, aidez-moi s'il vous plaît.", "道に迷いました。助けてください"],
    ["J'ai perdu mon sac.", "かばんをなくしました"], ["Allez à cette adresse, s'il vous plaît.", "この住所まで行ってください"], ["Je voudrais aller à la gare la plus proche.", "一番近い駅へ行きたいです"],
    ["Je voudrais faire l'enregistrement.", "チェックインしたいです"], ["Vérifiez la chambre, s'il vous plaît.", "部屋を確認してください"], ["Puis-je avoir une serviette en plus ?", "タオルを追加でもらえますか"],
    ["À quelle heure commence le petit déjeuner ?", "朝食は何時からですか"], ["Je voudrais annuler ma réservation.", "予約をキャンセルしたいです"], ["C'est bien par cette route ?", "この道で合っていますか"],
    ["Je voudrais acheter un billet.", "チケットを1枚買いたいです"], ["Puis-je être remboursé ?", "返金できますか"], ["Vous avez une taille un peu plus grande ?", "もう少し大きいサイズはありますか"],
    ["Ce quartier est sûr le soir ?", "この辺りは夜安全ですか"], ["Je dois aller aux urgences.", "救急外来に行く必要があります"], ["J'ai besoin des documents d'assurance.", "保険の書類が必要です"],
    ["Appelez un taxi, s'il vous plaît.", "タクシーを呼んでください"], ["Ce bus va à l'aéroport ?", "このバスは空港に行きますか"], ["Puis-je laisser mes bagages ici ?", "ここで荷物を預けられますか"],
    ["Sans coriandre, s'il vous plaît.", "パクチーを入れないでください"], ["Je voudrais un siège côté fenêtre.", "窓側の席に変えたいです"]
  ]
};

const koreanPhraseBoost4 = {
  practical: [
    ["알림이 너무 많아요.", "通知が多すぎます"], ["설정을 바꾸고 싶어요.", "設定を変えたいです"], ["비밀번호를 잊어버렸어요.", "パスワードを忘れました"],
    ["화면이 잘 안 보여요.", "画面がよく見えません"], ["파일이 열리지 않아요.", "ファイルが開きません"], ["인터넷 연결을 확인해 주세요.", "ネット接続を確認してください"],
    ["배터리가 거의 없어요.", "バッテリーがほとんどありません"], ["충전기를 빌릴 수 있나요?", "充電器を借りられますか"], ["메모에 적어 둘게요.", "メモに書いておきます"],
    ["주소를 다시 보내 주세요.", "住所をもう一度送ってください"], ["먼저 이유를 설명해 주세요.", "先に理由を説明してください"], ["이 방법으로 해 볼게요.", "この方法でやってみます"],
    ["어떤 선택이 좋아요?", "どの選択がいいですか"], ["검색어를 바꿔 볼게요.", "検索語を変えてみます"], ["기록이 남아 있나요?", "記録は残っていますか"],
    ["오늘 배운 내용을 복습할게요.", "今日学んだ内容を復習します"], ["발음을 천천히 들려 주세요.", "発音をゆっくり聞かせてください"], ["문장을 짧게 만들어 주세요.", "文を短くしてください"],
    ["단어를 예문으로 외울게요.", "単語を例文で覚えます"], ["차이를 표로 정리해 주세요.", "違いを表で整理してください"], ["실수한 부분을 표시해 주세요.", "間違えた部分を表示してください"],
    ["저장이 완료됐어요.", "保存が完了しました"], ["삭제해도 괜찮나요?", "削除しても大丈夫ですか"], ["잠깐 쉬었다가 계속할게요.", "少し休んでから続けます"],
    ["목표를 조금 낮출게요.", "目標を少し下げます"], ["다시 시작해도 될까요?", "もう一度始めてもいいですか"], ["천천히 따라 말할게요.", "ゆっくりまねして話します"],
    ["같은 문제를 다시 풀고 싶어요.", "同じ問題をもう一度解きたいです"], ["답을 보기 전에 생각할게요.", "答えを見る前に考えます"], ["어려운 단어만 모아 주세요.", "難しい単語だけ集めてください"]
  ],
  business: [
    ["회의 시간을 다시 잡겠습니다.", "会議時間を調整します"], ["자료를 회의 전에 공유하겠습니다.", "資料を会議前に共有します"], ["담당자를 정해 주세요.", "担当者を決めてください"],
    ["고객에게 보낼 문장을 확인해 주세요.", "顧客に送る文を確認してください"], ["계약 조건을 다시 보겠습니다.", "契約条件を見直します"], ["견적 금액을 비교해 보겠습니다.", "見積金額を比較します"],
    ["청구서 번호를 알려 주세요.", "請求書番号を教えてください"], ["재고가 언제 들어오나요?", "在庫はいつ入りますか"], ["예산 안에서 진행하겠습니다.", "予算内で進めます"],
    ["마감 전에 초안을 보내겠습니다.", "締切前に下書きを送ります"], ["위험 요소를 먼저 공유하겠습니다.", "リスクを先に共有します"], ["최종본을 오늘 보내겠습니다.", "最終版を今日送ります"],
    ["승인 여부를 오늘 확인하겠습니다.", "承認可否を今日確認します"], ["수정한 파일을 다시 보내겠습니다.", "修正したファイルを送り直します"], ["운영 절차를 문서로 남기겠습니다.", "運用手順を文書に残します"],
    ["공개 일정을 미리 공유하겠습니다.", "公開日程を事前に共有します"], ["검수 결과를 정리하겠습니다.", "検収結果を整理します"]
  ]
};

const chinesePhraseBoost4 = {
  practical: [
    ["通知太多了。", "通知が多すぎます"], ["我想改设置。", "設定を変えたいです"], ["我忘了密码。", "パスワードを忘れました"],
    ["屏幕看不太清楚。", "画面がよく見えません"], ["文件打不开。", "ファイルが開きません"], ["请确认网络连接。", "ネット接続を確認してください"],
    ["电池快没了。", "バッテリーがほとんどありません"], ["可以借充电器吗？", "充電器を借りられますか"], ["我先记在笔记里。", "メモに書いておきます"],
    ["请再发一次地址。", "住所をもう一度送ってください"], ["请先说明理由。", "先に理由を説明してください"], ["我试试这个方法。", "この方法でやってみます"],
    ["哪个选择比较好？", "どの選択がいいですか"], ["我换个搜索词试试。", "検索語を変えてみます"], ["记录还在吗？", "記録は残っていますか"],
    ["我复习今天学的内容。", "今日学んだ内容を復習します"], ["请慢慢播放发音。", "発音をゆっくり再生してください"], ["请把句子改短一点。", "文を短くしてください"],
    ["我用例句记单词。", "単語を例文で覚えます"], ["请用表格整理区别。", "違いを表で整理してください"], ["请标出错的地方。", "間違えた部分を表示してください"],
    ["保存完成了。", "保存が完了しました"], ["可以删除吗？", "削除しても大丈夫ですか"], ["我休息一下再继续。", "少し休んでから続けます"],
    ["我把目标调低一点。", "目標を少し下げます"], ["可以重新开始吗？", "もう一度始めてもいいですか"], ["我慢慢跟着说。", "ゆっくりまねして話します"],
    ["我想再做一次同样的问题。", "同じ問題をもう一度解きたいです"], ["我看答案前先想一下。", "答えを見る前に考えます"], ["请只整理难的单词。", "難しい単語だけ集めてください"]
  ],
  business: [
    ["我会重新安排会议时间。", "会議時間を調整します"], ["我会在会议前共享资料。", "資料を会議前に共有します"], ["请确定负责人。", "担当者を決めてください"],
    ["请确认发给客户的句子。", "顧客に送る文を確認してください"], ["我会重新看合同条件。", "契約条件を見直します"], ["我比较一下报价金额。", "見積金額を比較します"],
    ["请告诉我发票号码。", "請求書番号を教えてください"], ["库存什么时候到？", "在庫はいつ入りますか"], ["我会在预算内推进。", "予算内で進めます"],
    ["我会在截止日期前发送草稿。", "締切前に下書きを送ります"], ["我先共享风险点。", "リスクを先に共有します"], ["我今天发送最终版。", "最終版を今日送ります"],
    ["我今天确认是否批准。", "承認可否を今日確認します"], ["我会重新发送修改后的文件。", "修正したファイルを送り直します"], ["我把运营流程写成文档。", "運用手順を文書に残します"],
    ["我会提前共享发布时间。", "公開日程を事前に共有します"], ["我整理验收结果。", "検収結果を整理します"]
  ]
};

const frenchPhraseBoost4 = {
  practical: [
    ["Il y a trop de notifications.", "通知が多すぎます"], ["Je voudrais changer les réglages.", "設定を変えたいです"], ["J'ai oublié le mot de passe.", "パスワードを忘れました"],
    ["L'écran est difficile à voir.", "画面がよく見えません"], ["Le fichier ne s'ouvre pas.", "ファイルが開きません"], ["Vérifiez la connexion Internet, s'il vous plaît.", "ネット接続を確認してください"],
    ["La batterie est presque vide.", "バッテリーがほとんどありません"], ["Puis-je emprunter un chargeur ?", "充電器を借りられますか"], ["Je vais le noter dans le mémo.", "メモに書いておきます"],
    ["Renvoyez l'adresse, s'il vous plaît.", "住所をもう一度送ってください"], ["Expliquez d'abord la raison, s'il vous plaît.", "先に理由を説明してください"], ["Je vais essayer avec cette méthode.", "この方法でやってみます"],
    ["Quel choix est le meilleur ?", "どの選択がいいですか"], ["Je vais essayer un autre mot-clé.", "検索語を変えてみます"], ["L'historique est encore disponible ?", "記録は残っていますか"],
    ["Je vais réviser ce que j'ai appris aujourd'hui.", "今日学んだ内容を復習します"], ["Faites écouter la prononciation lentement.", "発音をゆっくり聞かせてください"], ["Raccourcissez la phrase, s'il vous plaît.", "文を短くしてください"],
    ["Je vais retenir le mot avec une phrase d'exemple.", "単語を例文で覚えます"], ["Mettez les différences dans un tableau.", "違いを表で整理してください"], ["Indiquez la partie incorrecte.", "間違えた部分を表示してください"],
    ["L'enregistrement est terminé.", "保存が完了しました"], ["Puis-je supprimer cela ?", "削除しても大丈夫ですか"], ["Je fais une petite pause puis je continue.", "少し休んでから続けます"],
    ["Je vais baisser un peu l'objectif.", "目標を少し下げます"], ["Puis-je recommencer ?", "もう一度始めてもいいですか"], ["Je vais répéter lentement.", "ゆっくりまねして話します"],
    ["Je voudrais refaire le même exercice.", "同じ問題をもう一度解きたいです"], ["Je vais réfléchir avant de voir la réponse.", "答えを見る前に考えます"], ["Regroupez seulement les mots difficiles.", "難しい単語だけ集めてください"]
  ],
  business: [
    ["Je vais réorganiser l'heure de la réunion.", "会議時間を調整します"], ["Je partagerai les documents avant la réunion.", "資料を会議前に共有します"], ["Désignez le responsable, s'il vous plaît.", "担当者を決めてください"],
    ["Vérifiez le message à envoyer au client.", "顧客に送る文を確認してください"], ["Je vais revoir les conditions du contrat.", "契約条件を見直します"], ["Je vais comparer les montants des devis.", "見積金額を比較します"],
    ["Donnez-moi le numéro de facture, s'il vous plaît.", "請求書番号を教えてください"], ["Quand le stock sera-t-il disponible ?", "在庫はいつ入りますか"], ["Nous avancerons dans le budget.", "予算内で進めます"],
    ["J'enverrai le brouillon avant la date limite.", "締切前に下書きを送ります"], ["Je partagerai d'abord les risques.", "リスクを先に共有します"], ["J'enverrai la version finale aujourd'hui.", "最終版を今日送ります"],
    ["Je vérifierai l'approbation aujourd'hui.", "承認可否を今日確認します"], ["Je renverrai le fichier corrigé.", "修正したファイルを送り直します"], ["Je garderai la procédure d'exploitation par écrit.", "運用手順を文書に残します"],
    ["Je partagerai le calendrier de publication à l'avance.", "公開日程を事前に共有します"], ["Je vais résumer les résultats de recette.", "検収結果を整理します"]
  ]
};

const koreanWordBoost5 = {
  basic: [
    ["항상", "いつも"], ["가끔", "ときどき"], ["먼저", "先に"], ["나중에", "あとで"], ["함께", "一緒に"],
    ["혼자", "一人で"], ["밖", "外"], ["안", "中"], ["위", "上"], ["아래", "下"],
    ["앞", "前"], ["뒤", "後ろ"], ["옆", "横"], ["가까이", "近くに"], ["멀리", "遠くに"],
    ["왼쪽", "左"], ["오른쪽", "右"], ["문제", "問題"], ["답", "答え"], ["질문", "質問"],
    ["소리", "音"], ["목소리", "声"], ["글자", "文字"], ["종이", "紙"], ["열쇠", "鍵"],
    ["지갑", "財布"], ["휴대폰", "スマホ"], ["가게", "店"], ["길", "道"], ["위치", "場所"],
    ["날", "日"], ["달력", "カレンダー"], ["주문", "注文"], ["계산", "会計"], ["선물", "プレゼント"],
    ["일", "仕事 / こと"], ["공부", "勉強"], ["휴식", "休み"], ["기쁨", "喜び"], ["슬픔", "悲しみ"],
    ["아프다", "痛い / 具合が悪い"], ["덥다", "暑い"], ["춥다", "寒い"], ["빠르다", "速い"], ["느리다", "遅い"],
    ["새롭다", "新しい"], ["오래되다", "古い"], ["가깝다", "近い"], ["멀다", "遠い"], ["같다", "同じだ"],
    ["다르다", "違う"], ["있다", "ある / いる"], ["없다", "ない / いない"], ["알다", "知っている"], ["모르다", "分からない"]
  ],
  practical: [
    ["집중", "集中"], ["상담", "相談"], ["확인사항", "確認事項"], ["필수", "必須"], ["선택사항", "任意項目"],
    ["오류", "エラー"], ["원인", "原因"], ["해결", "解決"], ["개선점", "改善点"], ["불만", "不満"],
    ["만족", "満足"], ["부담", "負担"], ["여유", "余裕"], ["목록", "リスト"], ["분류", "分類"],
    ["기준", "基準"], ["조건", "条件"], ["제한", "制限"], ["허용", "許可"], ["금지", "禁止"],
    ["변경", "変更"], ["추가", "追加"], ["삭제", "削除"], ["복사", "コピー"], ["공유", "共有"],
    ["입력", "入力"], ["출력", "出力"], ["선택지", "選択肢"], ["정답", "正解"], ["오답", "不正解"],
    ["속도조절", "速度調整"], ["재생", "再生"], ["정지", "停止"], ["이전", "前"], ["다음", "次"],
    ["알림설정", "通知設定"], ["화면표시", "画面表示"], ["음성", "音声"], ["반응", "反応"], ["기록관리", "記録管理"],
    ["안정적", "安定した"], ["불안정", "不安定な"], ["자세하다", "詳しい"], ["간단하다", "簡単だ"], ["복잡하다", "複雑だ"],
    ["유용하다", "役に立つ"], ["익숙해지다", "慣れる"], ["헷갈리다", "紛らわしい"], ["놓치다", "見逃す"], ["확인받다", "確認してもらう"]
  ],
  business: [
    ["매출총액", "総売上"], ["이익", "利益"], ["손실", "損失"], ["원가", "原価"], ["단가", "単価"],
    ["세금", "税金"], ["계좌", "口座"], ["입금", "入金"], ["출금", "出金"], ["정산", "精算"],
    ["발주", "発注"], ["수주", "受注"], ["구매", "購入"], ["판매", "販売"], ["납기", "納期"],
    ["검품", "検品"], ["반품", "返品"], ["교환", "交換"], ["재발송", "再送"], ["지급일", "支払日"],
    ["담당부서", "担当部署"], ["책임자", "責任者"], ["관리자", "管理者"], ["승인자", "承認者"], ["요청자", "依頼者"],
    ["회의실", "会議室"], ["초대", "招待"], ["참석", "参加"], ["불참", "不参加"], ["결석", "欠席"],
    ["안건", "議題"], ["결론", "結論"], ["과제", "課題"], ["일정표", "予定表"], ["진행률", "進捗率"],
    ["우선도", "優先度"], ["기밀", "機密"], ["보류", "保留"], ["완료", "完了"], ["미완료", "未完了"],
    ["긴급", "緊急"], ["일반", "通常"], ["정기", "定期"], ["임시", "臨時"], ["사내", "社内"],
    ["외부", "外部"], ["검토중", "検討中"], ["확정", "確定"], ["협력", "協力"], ["협상", "交渉"]
  ],
  travel: [
    ["여행", "旅行"], ["관광", "観光"], ["일정", "旅程"], ["지도앱", "地図アプリ"], ["목적지", "目的地"],
    ["출발지", "出発地"], ["도착지", "到着地"], ["국내선", "国内線"], ["국제선", "国際線"], ["항공사", "航空会社"],
    ["수속", "手続き"], ["보안검색", "保安検査"], ["탑승", "搭乗"], ["연착", "遅延"], ["취소", "キャンセル"],
    ["매표", "発券"], ["교통카드", "交通カード"], ["노선", "路線"], ["정류장", "停留所"], ["요금", "料金"],
    ["숙소예약", "宿泊予約"], ["객실", "客室"], ["싱글룸", "シングルルーム"], ["더블룸", "ダブルルーム"], ["금연실", "禁煙室"],
    ["흡연실", "喫煙室"], ["전망", "眺め"], ["엘리베이터", "エレベーター"], ["세탁", "洗濯"], ["건조기", "乾燥機"],
    ["식사", "食事"], ["아침식사", "朝食"], ["점심식사", "昼食"], ["저녁식사", "夕食"], ["음료", "飲み物"],
    ["생수", "飲料水"], ["매운맛", "辛さ"], ["채식", "ベジタリアン"], ["알레르기", "アレルギー"], ["계산서", "伝票"],
    ["영업시간", "営業時間"], ["면세점", "免税店"], ["환율", "為替レート"], ["영수증", "領収書"], ["진료", "診察"],
    ["처방전", "処方箋"], ["분실신고", "紛失届"], ["도난", "盗難"], ["대사관", "大使館"], ["응급", "緊急"]
  ]
};

const chineseWordBoost5 = {
  basic: [
    ["总是", "いつも"], ["有时", "ときどき"], ["先", "先に"], ["以后", "あとで"], ["一起", "一緒に"],
    ["一个人", "一人で"], ["外面", "外"], ["里面", "中"], ["上面", "上"], ["下面", "下"],
    ["前面", "前"], ["后面", "後ろ"], ["旁边", "横"], ["远", "遠い"], ["一样", "同じ"],
    ["不同", "違う"], ["声音", "音 / 声"], ["文字", "文字"], ["纸", "紙"], ["钥匙", "鍵"],
    ["钱包", "財布"], ["手机", "スマホ"], ["地方", "場所"], ["日期", "日付"], ["日历", "カレンダー"],
    ["订单", "注文"], ["付款", "支払い"], ["礼物", "プレゼント"], ["学习", "勉強"], ["休息", "休み"],
    ["高兴", "うれしい"], ["难过", "悲しい"], ["疼", "痛い"], ["热", "暑い / 熱い"], ["冷", "寒い / 冷たい"],
    ["新", "新しい"], ["旧", "古い"], ["近", "近い"], ["知道", "知っている"], ["不知道", "分からない"],
    ["有", "ある / いる"], ["没有", "ない / いない"], ["全部", "全部"], ["一半", "半分"], ["附近", "近く"],
    ["马上", "すぐに"], ["刚才", "さっき"], ["已经", "すでに"], ["还", "まだ"], ["也", "〜も"],
    ["再", "もう一度"], ["只", "〜だけ"], ["很", "とても"], ["比较", "比較的"], ["最", "最も"]
  ],
  practical: [
    ["集中", "集中"], ["咨询", "相談"], ["确认事项", "確認事項"], ["必需", "必須"], ["可选", "任意"],
    ["错误", "エラー"], ["原因", "原因"], ["解决", "解決"], ["改善点", "改善点"], ["不满", "不満"],
    ["满意", "満足"], ["负担", "負担"], ["余裕", "余裕"], ["列表", "リスト"], ["分类", "分類"],
    ["标准", "基準"], ["条件", "条件"], ["限制", "制限"], ["允许", "許可"], ["禁止", "禁止"],
    ["变更", "変更"], ["添加", "追加"], ["复制", "コピー"], ["分享", "共有"], ["输入", "入力"],
    ["输出", "出力"], ["选项", "選択肢"], ["正确答案", "正解"], ["错误答案", "不正解"], ["播放", "再生"],
    ["停止", "停止"], ["上一个", "前"], ["下一个", "次"], ["音频", "音声"], ["反应", "反応"],
    ["稳定", "安定した"], ["不稳定", "不安定な"], ["详细", "詳しい"], ["复杂", "複雑な"], ["有用", "役に立つ"],
    ["习惯", "慣れる"], ["容易混淆", "紛らわしい"], ["错过", "見逃す"], ["重新", "再び"], ["自动", "自動"]
  ],
  business: [
    ["总销售额", "総売上"], ["利润", "利益"], ["损失", "損失"], ["成本", "原価"], ["单价", "単価"],
    ["税金", "税金"], ["账户", "口座"], ["到账", "入金"], ["提现", "出金"], ["结算", "精算"],
    ["下单", "発注"], ["接单", "受注"], ["采购", "購入"], ["销售", "販売"], ["交货期", "納期"],
    ["质检", "検品"], ["退货", "返品"], ["补发", "再送"], ["付款日", "支払日"], ["负责部门", "担当部署"],
    ["责任人", "責任者"], ["管理员", "管理者"], ["审批人", "承認者"], ["请求人", "依頼者"], ["邀请", "招待"],
    ["参加", "参加"], ["缺席", "欠席"], ["结论", "結論"], ["课题", "課題"], ["完成率", "達成率"],
    ["优先度", "優先度"], ["机密", "機密"], ["保留", "保留"], ["已完成", "完了"], ["未完成", "未完了"],
    ["紧急", "緊急"], ["普通", "通常"], ["定期", "定期"], ["临时", "臨時"], ["内部", "社内"],
    ["外部", "外部"], ["审核中", "検討中"], ["确定", "確定"], ["合作", "協力"], ["谈判", "交渉"],
    ["交接", "引き継ぎ"], ["培训", "研修"], ["评价", "評価"], ["投诉", "クレーム"], ["应对", "対応"]
  ],
  travel: [
    ["旅行", "旅行"], ["观光", "観光"], ["行程", "旅程"], ["目的地", "目的地"], ["出发地", "出発地"],
    ["到达地", "到着地"], ["国内线", "国内線"], ["国际线", "国際線"], ["航空公司", "航空会社"], ["手续", "手続き"],
    ["安检", "保安検査"], ["登机", "搭乗"], ["延误", "遅延"], ["取消", "キャンセル"], ["出票", "発券"],
    ["交通卡", "交通カード"], ["路线", "路線"], ["站点", "停留所"], ["票价", "料金"], ["住宿预订", "宿泊予約"],
    ["单人间", "シングルルーム"], ["双人间", "ダブルルーム"], ["禁烟房", "禁煙室"], ["吸烟房", "喫煙室"], ["景色", "眺め"],
    ["电梯", "エレベーター"], ["洗衣", "洗濯"], ["烘干机", "乾燥機"], ["餐食", "食事"], ["饮料", "飲み物"],
    ["矿泉水", "飲料水"], ["辣度", "辛さ"], ["素食", "ベジタリアン"], ["过敏", "アレルギー"], ["账单", "伝票"],
    ["免税店", "免税店"], ["汇率", "為替レート"], ["诊疗", "診察"], ["处方", "処方箋"], ["失窃", "盗難"],
    ["大使馆", "大使館"], ["紧急", "緊急"], ["问讯", "問い合わせ"], ["排队", "行列"], ["转乘", "乗り換え"]
  ]
};

const frenchWordBoost5 = {
  basic: [
    ["toujours", "いつも"], ["parfois", "ときどき"], ["d'abord", "先に"], ["plus tard", "あとで"], ["ensemble", "一緒に"],
    ["seul", "一人で"], ["dehors", "外"], ["dedans", "中"], ["dessus", "上"], ["dessous", "下"],
    ["avant", "前"], ["après", "後"], ["côté", "横"], ["loin", "遠い"], ["pareil", "同じ"],
    ["différent", "違う"], ["voix", "声"], ["lettre", "文字"], ["papier", "紙"], ["portefeuille", "財布"],
    ["smartphone", "スマホ"], ["lieu", "場所"], ["date", "日付"], ["calendrier", "カレンダー"], ["cadeau", "プレゼント"],
    ["repos", "休み"], ["joie", "喜び"], ["tristesse", "悲しみ"], ["malade", "具合が悪い"], ["chaud", "暑い / 熱い"],
    ["froid", "寒い / 冷たい"], ["nouveau", "新しい"], ["ancien", "古い"], ["proche", "近い"], ["savoir", "知っている"],
    ["ignorer", "知らない"], ["tout", "全部"], ["moitié", "半分"], ["bientôt", "もうすぐ"], ["déjà", "すでに"],
    ["encore", "まだ / もう一度"], ["seulement", "〜だけ"], ["très", "とても"], ["plutôt", "比較的"], ["mieux", "より良く"],
    ["pire", "より悪く"], ["facilement", "簡単に"], ["souvent", "よく"], ["rarement", "めったにない"], ["possible", "可能な"]
  ],
  practical: [
    ["concentration", "集中"], ["consultation", "相談"], ["obligatoire", "必須"], ["facultatif", "任意"], ["erreur", "エラー"],
    ["cause", "原因"], ["solution", "解決策"], ["insatisfaction", "不満"], ["satisfaction", "満足"], ["charge", "負担"],
    ["marge", "余裕"], ["liste", "リスト"], ["catégorie", "分類"], ["critère", "基準"], ["condition", "条件"],
    ["limite", "制限"], ["permission", "許可"], ["interdiction", "禁止"], ["changement", "変更"], ["ajout", "追加"],
    ["copie", "コピー"], ["entrée", "入力"], ["sortie", "出力"], ["option", "選択肢"], ["lecture", "再生"],
    ["arrêt", "停止"], ["précédent", "前"], ["suivant", "次"], ["audio", "音声"], ["réaction", "反応"],
    ["stable", "安定した"], ["instable", "不安定な"], ["détaillé", "詳しい"], ["complexe", "複雑な"], ["utile", "役に立つ"],
    ["s'habituer", "慣れる"], ["confus", "紛らわしい"], ["manquer", "見逃す"], ["automatique", "自動"], ["manuel", "手動"]
  ],
  business: [
    ["bénéfice", "利益"], ["perte", "損失"], ["prix", "単価"], ["impôt", "税金"], ["compte", "口座"],
    ["virement", "振込"], ["commande", "発注 / 注文"], ["achat", "購入"], ["vente", "販売"], ["inspection", "検品"],
    ["retour", "返品"], ["renvoi", "再送"], ["paiement", "支払い"], ["manager", "管理者"], ["invitation", "招待"],
    ["participation", "参加"], ["absence", "欠席"], ["conclusion", "結論"], ["tâche", "課題"], ["confidentiel", "機密"],
    ["suspendu", "保留"], ["terminé", "完了"], ["urgent", "緊急"], ["normal", "通常"], ["régulier", "定期"],
    ["temporaire", "臨時"], ["interne", "社内"], ["externe", "外部"], ["négociation", "交渉"], ["transmission", "引き継ぎ"],
    ["formation", "研修"], ["évaluation", "評価"], ["réclamation", "クレーム"], ["réponse", "対応 / 返答"], ["ressources", "リソース"],
    ["analyse", "分析"], ["prévision", "予測"], ["rentable", "採算が合う"], ["priorité", "優先順位"], ["signature", "署名"]
  ],
  travel: [
    ["voyage", "旅行"], ["tourisme", "観光"], ["destination", "目的地"], ["départ", "出発"], ["arrivée", "到着"],
    ["compagnie", "航空会社"], ["formalité", "手続き"], ["embarquement", "搭乗"], ["retard", "遅延"], ["annulation", "キャンセル"],
    ["trajet", "路線"], ["arrêt", "停留所"], ["tarif", "料金"], ["réservation", "予約"], ["chambre", "客室"],
    ["ascenseur", "エレベーター"], ["lessive", "洗濯"], ["boisson", "飲み物"], ["épicé", "辛い"], ["allergie", "アレルギー"],
    ["addition", "伝票 / 会計"], ["détaxe", "免税"], ["taux", "為替レート"], ["consultation", "診察"], ["ordonnance", "処方箋"],
    ["vol", "盗難 / 便"], ["ambassade", "大使館"], ["urgence", "緊急"], ["file", "行列"], ["correspondance", "乗り換え"],
    ["douane", "税関"], ["contrôle", "検査"], ["bagage", "荷物"], ["valise", "スーツケース"], ["passeport", "パスポート"],
    ["billet", "チケット"], ["guichet", "窓口"], ["station", "駅 / 停留所"], ["plage", "ビーチ"], ["souvenir", "お土産"]
  ]
};

const koreanWordBoost6 = {
  basic: [
    ["얼굴", "顔"], ["목", "首 / 喉"], ["어깨", "肩"], ["등", "背中 / など"], ["배", "お腹 / 船"],
    ["다리", "脚 / 橋"], ["마을", "町 / 村"], ["건물", "建物"], ["층", "階"], ["계단", "階段"],
    ["생일", "誕生日"], ["나이", "年齢"], ["입다", "着る"], ["벗다", "脱ぐ"], ["씻다", "洗う"],
    ["자다", "寝る"], ["일어나다", "起きる"], ["만나다", "会う"], ["받다", "受け取る"], ["사다", "買う"],
    ["팔다", "売る"], ["열다", "開ける"], ["닫다", "閉める"], ["앉다", "座る"], ["서다", "立つ"],
    ["걷다", "歩く"], ["달리다", "走る"], ["묻다", "尋ねる"], ["대답하다", "答える"], ["기다리다", "待つ"]
  ],
  practical: [
    ["안내", "案内"], ["신청", "申請"], ["등록", "登録"], ["취소", "キャンセル"], ["확대", "拡大"],
    ["축소", "縮小"], ["이동", "移動"], ["붙여넣기", "貼り付け"], ["업로드", "アップロード"], ["다운로드", "ダウンロード"],
    ["알림음", "通知音"], ["권장", "推奨"], ["안심", "安心"], ["피로", "疲れ"], ["회복", "回復"],
    ["증가", "増加"], ["감소", "減少"], ["비율", "割合"], ["예외", "例外"], ["세부사항", "詳細"],
    ["요약", "要約"], ["주의", "注意"], ["안전", "安全"], ["위험", "危険"], ["습관", "習慣"],
    ["가능성", "可能性"], ["필요성", "必要性"], ["차단", "ブロック"], ["해제", "解除"], ["동의", "同意"]
  ],
  business: [
    ["견적", "見積もり"], ["출하", "出荷"], ["배송", "配送"], ["환불", "返金"], ["고객사", "顧客企業"],
    ["협력사", "協力会社"], ["권한", "権限"], ["보안", "セキュリティ"], ["의사록", "議事録"], ["결재", "決裁"],
    ["반려", "差し戻し"], ["우선순위", "優先順位"], ["마감", "締切"], ["계약", "契約"], ["제휴", "提携"],
    ["홍보", "広報"], ["광고", "広告"], ["시장", "市場"], ["수요", "需要"], ["공급", "供給"],
    ["가격", "価格"], ["할인율", "割引率"], ["성과", "成果"], ["실적", "実績"], ["분석", "分析"],
    ["예측", "予測"], ["개발", "開発"], ["출시", "リリース"], ["운영", "運用"], ["장애", "障害"]
  ],
  travel: [
    ["지연", "遅延"], ["요금", "料金"], ["세금", "税金"], ["수수료", "手数料"], ["면세", "免税"],
    ["환전소", "両替所"], ["개찰구", "改札"], ["종점", "終点"], ["좌석", "座席"], ["통로", "通路"],
    ["비상구", "非常口"], ["주소", "住所"], ["현지", "現地"], ["입장권", "入場券"], ["약국", "薬局"],
    ["휴대품", "携帯品"], ["분실물", "忘れ物"], ["신분증", "身分証"], ["안내소", "案内所"], ["대합실", "待合室"],
    ["출입구", "出入口"], ["왕복", "往復"], ["편도", "片道"], ["환승역", "乗換駅"], ["기차역", "駅"],
    ["해변", "海辺"], ["전망대", "展望台"], ["박물관", "博物館"], ["미술관", "美術館"], ["축제", "祭り"]
  ]
};

const chineseWordBoost6 = {
  basic: [
    ["身体", "体"], ["脸", "顔"], ["脖子", "首"], ["肩膀", "肩"], ["背", "背中"],
    ["肚子", "お腹"], ["腿", "脚"], ["城镇", "町"], ["道路", "道"], ["建筑", "建物"],
    ["楼层", "階"], ["楼梯", "階段"], ["生日", "誕生日"], ["年龄", "年齢"], ["穿", "着る"],
    ["脱", "脱ぐ"], ["洗", "洗う"], ["睡觉", "寝る"], ["起床", "起きる"], ["见面", "会う"],
    ["收到", "受け取る"], ["买", "買う"], ["卖", "売る"], ["打开", "開ける"], ["关闭", "閉める"],
    ["坐", "座る"], ["站", "立つ"], ["走路", "歩く"], ["跑", "走る"], ["回答", "答える"]
  ],
  practical: [
    ["申请", "申請"], ["注册", "登録"], ["更改", "変更"], ["放大", "拡大"], ["缩小", "縮小"],
    ["移动", "移動"], ["粘贴", "貼り付け"], ["上传", "アップロード"], ["下载", "ダウンロード"], ["推荐", "推奨"],
    ["安心", "安心"], ["疲劳", "疲れ"], ["恢复", "回復"], ["增加", "増加"], ["减少", "減少"],
    ["比例", "割合"], ["例外", "例外"], ["详情", "詳細"], ["总结", "要約"], ["注意", "注意"],
    ["安全", "安全"], ["危险", "危険"], ["习惯", "習慣"], ["可能性", "可能性"], ["必要性", "必要性"],
    ["解除", "解除"], ["同意", "同意"], ["拒绝", "拒否"], ["提醒", "リマインド"], ["保存", "保存"]
  ],
  business: [
    ["报价", "見積もり"], ["发货", "出荷"], ["配送", "配送"], ["换货", "交換"], ["退款", "返金"],
    ["权限", "権限"], ["安全", "セキュリティ"], ["会议室", "会議室"], ["审批", "承認 / 決裁"], ["驳回", "差し戻し"],
    ["截止", "締切"], ["合同", "契約"], ["合作", "提携"], ["宣传", "広報"], ["广告", "広告"],
    ["市场", "市場"], ["需求", "需要"], ["供应", "供給"], ["价格", "価格"], ["折扣率", "割引率"],
    ["业绩", "実績"], ["分析", "分析"], ["预测", "予測"], ["开发", "開発"], ["发布", "リリース"],
    ["运营", "運用"], ["故障", "障害"], ["竞争", "競争"], ["战略", "戦略"], ["效率", "効率"]
  ],
  travel: [
    ["税", "税金"], ["手续费", "手数料"], ["兑换处", "両替所"], ["检票口", "改札"], ["终点", "終点"],
    ["座位", "座席"], ["过道", "通路"], ["紧急出口", "非常口"], ["地址", "住所"], ["当地", "現地"],
    ["门票", "入場券"], ["药店", "薬局"], ["证件", "身分証"], ["问讯处", "案内所"], ["候车室", "待合室"],
    ["出入口", "出入口"], ["往返", "往復"], ["单程", "片道"], ["换乘站", "乗換駅"], ["火车站", "駅"],
    ["海边", "海辺"], ["观景台", "展望台"], ["博物馆", "博物館"], ["美术馆", "美術館"], ["节日", "祭り"],
    ["夜市", "夜市"], ["小费", "チップ"], ["押金", "保証金"], ["海关", "税関"], ["签证", "ビザ"]
  ]
};

const frenchWordBoost6 = {
  basic: [
    ["corps", "体"], ["visage", "顔"], ["cou", "首"], ["épaule", "肩"], ["dos", "背中"],
    ["ventre", "お腹"], ["jambe", "脚"], ["ville", "町 / 都市"], ["route", "道"], ["bâtiment", "建物"],
    ["étage", "階"], ["escalier", "階段"], ["anniversaire", "誕生日"], ["âge", "年齢"], ["porter", "着る"],
    ["enlever", "脱ぐ"], ["laver", "洗う"], ["dormir", "寝る"], ["réveiller", "起こす"], ["rencontrer", "会う"],
    ["recevoir", "受け取る"], ["acheter", "買う"], ["vendre", "売る"], ["ouvrir", "開ける"], ["fermer", "閉める"],
    ["asseoir", "座らせる"], ["debout", "立って"], ["marcher", "歩く"], ["courir", "走る"], ["répondre", "答える"]
  ],
  practical: [
    ["demande", "申請 / 依頼"], ["inscription", "登録"], ["annulation", "キャンセル"], ["agrandir", "拡大する"], ["réduire", "縮小する"],
    ["déplacer", "移動する"], ["copier", "コピーする"], ["coller", "貼り付ける"], ["envoyer", "送る"], ["recommandation", "推奨"],
    ["permission", "許可"], ["soulagement", "安心"], ["fatigue", "疲れ"], ["récupération", "回復"], ["augmentation", "増加"],
    ["diminution", "減少"], ["exception", "例外"], ["détail", "詳細"], ["synthèse", "要約"], ["attention", "注意"],
    ["danger", "危険"], ["habitude", "習慣"], ["possibilité", "可能性"], ["nécessité", "必要性"], ["refus", "拒否"],
    ["accord", "同意"], ["rappel", "リマインド"], ["sauvegarde", "保存"], ["connexion", "接続"], ["affichage", "表示"]
  ],
  business: [
    ["devis", "見積もり"], ["expédition", "出荷"], ["livraison", "配送"], ["échange", "交換"], ["remboursement", "返金"],
    ["autorisation", "権限"], ["sécurité", "セキュリティ"], ["approbation", "承認"], ["priorité", "優先順位"], ["contrat", "契約"],
    ["partenariat", "提携"], ["communication", "広報"], ["publicité", "広告"], ["marché", "市場"], ["demande", "需要"],
    ["offre", "供給"], ["prix", "価格 / 単価"], ["réduction", "割引"], ["performance", "実績"], ["analyse", "分析"],
    ["prévision", "予測"], ["développement", "開発"], ["lancement", "リリース"], ["exploitation", "運用"], ["incident", "障害"],
    ["concurrence", "競争"], ["stratégie", "戦略"], ["efficacité", "効率"], ["facture", "請求書"], ["signature", "署名"]
  ],
  travel: [
    ["taxe", "税金"], ["commission", "手数料"], ["détaxe", "免税"], ["portique", "改札"], ["terminus", "終点"],
    ["siège", "座席"], ["couloir", "通路"], ["sortie", "出口"], ["adresse", "住所"], ["local", "現地の"],
    ["billet", "チケット / 入場券"], ["pharmacie", "薬局"], ["assurance", "保険"], ["identité", "身分"], ["guichet", "窓口"],
    ["aller-retour", "往復"], ["aller", "片道"], ["correspondance", "乗り換え"], ["gare", "駅"], ["plage", "海辺"],
    ["musée", "博物館"], ["festival", "祭り"], ["pourboire", "チップ"], ["caution", "保証金"], ["douane", "税関"],
    ["visa", "ビザ"], ["urgence", "緊急"], ["ambassade", "大使館"], ["bagage", "荷物"], ["valise", "スーツケース"]
  ]
};

const koreanPhraseBoost5 = {
  basic: [
    ["오늘은 조금 일찍 일어났어요.", "今日は少し早く起きました"], ["이 가게는 역에서 가까워요.", "この店は駅から近いです"], ["나중에 전화할게요.", "あとで電話します"],
    ["사진을 한 장 찍었어요.", "写真を一枚撮りました"], ["새 신발을 샀어요.", "新しい靴を買いました"], ["주말에는 집에서 쉴 거예요.", "週末は家で休みます"],
    ["영화를 보러 갈 거예요.", "映画を見に行きます"], ["음악을 들으면서 걸어요.", "音楽を聞きながら歩きます"], ["이 번호를 기억하고 있어요.", "この番号を覚えています"],
    ["가격을 확인하고 살게요.", "値段を確認してから買います"], ["아침에 커피를 마셔요.", "朝にコーヒーを飲みます"], ["밤에는 일찍 잘게요.", "夜は早めに寝ます"],
    ["공원에서 조금 걸었어요.", "公園で少し歩きました"], ["오늘 요리를 만들었어요.", "今日は料理を作りました"]
  ],
  practical: [
    ["오늘 할 일을 먼저 정리할게요.", "今日やることを先に整理します"], ["중요한 것부터 시작할게요.", "重要なものから始めます"], ["잠깐만 시간을 주세요.", "少し待ってください"],
    ["지금 바로 확인해 볼게요.", "今すぐ確認してみます"], ["이 방법이 더 쉬워요.", "この方法のほうが簡単です"], ["다른 방법도 시도해 볼게요.", "別の方法も試してみます"],
    ["어디부터 보면 좋을까요?", "どこから見ればいいですか"], ["이 부분만 다시 해 볼게요.", "この部分だけもう一度やってみます"], ["메시지를 짧게 보낼게요.", "メッセージを短く送ります"],
    ["주소를 지도에서 확인할게요.", "住所を地図で確認します"], ["비밀번호를 새로 만들게요.", "パスワードを新しく作ります"], ["화면을 아래로 내려 주세요.", "画面を下にスクロールしてください"],
    ["파일 이름을 바꿀게요.", "ファイル名を変えます"], ["인터넷이 느려요.", "インターネットが遅いです"], ["소리가 잘 안 들려요.", "音がよく聞こえません"],
    ["글자가 너무 작아요.", "文字が小さすぎます"], ["다시 저장해 볼게요.", "もう一度保存してみます"], ["오늘 목표까지 조금 남았어요.", "今日の目標まで少し残っています"],
    ["이 화면을 조금 더 보기 쉽게 할게요.", "この画面を少し見やすくします"], ["소리를 한 번 더 들을게요.", "もう一度音声を聞きます"], ["다음에는 짧은 문장으로 연습할게요.", "次は短い文で練習します"],
    ["모르는 단어에 표시해 둘게요.", "分からない単語に印を付けます"], ["오늘 복습을 먼저 끝낼게요.", "今日の復習を先に終わらせます"], ["답을 보지 않고 생각할게요.", "答えを見ずに考えます"],
    ["예문을 제 말로 바꿔 볼게요.", "例文を自分の言葉に変えます"], ["저장이 됐는지 확인할게요.", "保存できたか確認します"], ["틀린 이유를 메모할게요.", "間違えた理由をメモします"],
    ["같은 표현을 소리 내어 말할게요.", "同じ表現を声に出します"], ["잘 안 들리면 속도를 낮출게요.", "聞こえないときは速度を下げます"], ["화면을 위로 올려 주세요.", "画面を上に戻してください"],
    ["마지막에 어려운 단어를 다시 볼게요.", "最後に苦手な単語を見直します"]
  ],
  business: [
    ["회의록을 오늘 안에 공유하겠습니다.", "議事録を今日中に共有します"], ["자료의 최신 버전을 보내 주세요.", "資料の最新版を送ってください"], ["담당자에게 먼저 확인하겠습니다.", "担当者に先に確認します"],
    ["고객 답변은 짧게 정리하겠습니다.", "顧客への回答は短く整理します"], ["계약 조건에 빠진 부분이 있습니다.", "契約条件に抜けがあります"], ["예산을 넘지 않도록 조정하겠습니다.", "予算を超えないよう調整します"],
    ["재고가 부족하면 대안을 찾겠습니다.", "在庫が足りない場合は代案を探します"], ["공개 전에 최종 확인을 하겠습니다.", "公開前に最終確認します"], ["문제가 생기면 바로 공유하겠습니다.", "問題が起きたらすぐ共有します"]
  ]
};

const chinesePhraseBoost5 = {
  basic: [
    ["今天我起得有点早。", "今日は少し早く起きました"], ["这家店离车站很近。", "この店は駅から近いです"], ["我待会儿打电话。", "あとで電話します"],
    ["我拍了一张照片。", "写真を一枚撮りました"], ["我买了新鞋。", "新しい靴を買いました"], ["周末我在家休息。", "週末は家で休みます"],
    ["我去看电影。", "映画を見に行きます"], ["我一边听音乐一边走路。", "音楽を聞きながら歩きます"], ["我记得这个号码。", "この番号を覚えています"],
    ["我确认价格后再买。", "値段を確認してから買います"], ["早上我喝咖啡。", "朝にコーヒーを飲みます"], ["晚上我早点睡。", "夜は早めに寝ます"],
    ["我在公园走了一会儿。", "公園で少し歩きました"], ["今天我做了菜。", "今日は料理を作りました"]
  ],
  practical: [
    ["我先整理今天要做的事。", "今日やることを先に整理します"], ["我先从重要的开始。", "重要なものから始めます"], ["请给我一点时间。", "少し待ってください"],
    ["我现在马上确认。", "今すぐ確認してみます"], ["这个方法更简单。", "この方法のほうが簡単です"], ["我也试试别的方法。", "別の方法も試してみます"],
    ["应该从哪里开始看？", "どこから見ればいいですか"], ["我只重新做这一部分。", "この部分だけもう一度やってみます"], ["我会把消息发短一点。", "メッセージを短く送ります"],
    ["我在地图上确认地址。", "住所を地図で確認します"], ["我重新设置密码。", "パスワードを新しく作ります"], ["请把屏幕往下滑。", "画面を下にスクロールしてください"],
    ["我改一下文件名。", "ファイル名を変えます"], ["网络很慢。", "インターネットが遅いです"], ["声音听不太清楚。", "音がよく聞こえません"],
    ["字太小了。", "文字が小さすぎます"], ["我再保存一次。", "もう一度保存してみます"], ["离今天的目标还差一点。", "今日の目標まで少し残っています"],
    ["我把这个画面调得更容易看。", "この画面を少し見やすくします"], ["我再听一遍声音。", "もう一度音声を聞きます"], ["下一次我用短句练习。", "次は短い文で練習します"],
    ["我把不懂的单词标出来。", "分からない単語に印を付けます"], ["我先完成今天的复习。", "今日の復習を先に終わらせます"], ["我不看答案，先想一下。", "答えを見ずに考えます"],
    ["我把例句改成自己的说法。", "例文を自分の言葉に変えます"], ["我确认一下有没有保存好。", "保存できたか確認します"], ["我把错的原因记下来。", "間違えた理由をメモします"],
    ["我把同一个表达说出声。", "同じ表現を声に出します"], ["听不清时我把速度调慢。", "聞こえないときは速度を下げます"], ["请把画面往上滑。", "画面を上に戻してください"],
    ["最后我再看一遍不熟的单词。", "最後に苦手な単語を見直します"]
  ],
  business: [
    ["我今天内共享会议记录。", "議事録を今日中に共有します"], ["请发送资料的最新版。", "資料の最新版を送ってください"], ["我先和负责人确认。", "担当者に先に確認します"],
    ["给客户的回复我会简短整理。", "顧客への回答は短く整理します"], ["合同条件里有遗漏。", "契約条件に抜けがあります"], ["我会调整到不超过预算。", "予算を超えないよう調整します"],
    ["库存不足时我会找替代方案。", "在庫が足りない場合は代案を探します"], ["发布前我会做最终确认。", "公開前に最終確認します"], ["如果出现问题，我会马上共享。", "問題が起きたらすぐ共有します"]
  ]
};

const frenchPhraseBoost5 = {
  basic: [
    ["Aujourd'hui, je me suis levé un peu tôt.", "今日は少し早く起きました"], ["Ce magasin est près de la gare.", "この店は駅から近いです"], ["J'appellerai plus tard.", "あとで電話します"],
    ["J'ai pris une photo.", "写真を一枚撮りました"], ["J'ai acheté de nouvelles chaussures.", "新しい靴を買いました"], ["Le week-end, je me repose à la maison.", "週末は家で休みます"],
    ["Je vais voir un film.", "映画を見に行きます"], ["Je marche en écoutant de la musique.", "音楽を聞きながら歩きます"], ["Je me souviens de ce numéro.", "この番号を覚えています"],
    ["Je vérifie le prix avant d'acheter.", "値段を確認してから買います"], ["Je bois du café le matin.", "朝にコーヒーを飲みます"], ["Je me couche tôt le soir.", "夜は早めに寝ます"],
    ["Aujourd'hui, j'ai cuisiné un plat.", "今日は料理を作りました"]
  ],
  practical: [
    ["Je vais d'abord organiser les tâches d'aujourd'hui.", "今日やることを先に整理します"], ["Je commence par le plus important.", "重要なものから始めます"], ["Donnez-moi juste un peu de temps.", "少し待ってください"],
    ["Je vais vérifier tout de suite.", "今すぐ確認してみます"], ["Cette méthode est plus simple.", "この方法のほうが簡単です"], ["Je vais essayer une autre méthode aussi.", "別の方法も試してみます"],
    ["Par où dois-je commencer ?", "どこから見ればいいですか"], ["Je vais refaire seulement cette partie.", "この部分だけもう一度やってみます"], ["J'enverrai un message court.", "メッセージを短く送ります"],
    ["Je vérifie l'adresse sur la carte.", "住所を地図で確認します"], ["Je vais créer un nouveau mot de passe.", "パスワードを新しく作ります"], ["Faites défiler l'écran vers le bas.", "画面を下にスクロールしてください"],
    ["Je vais changer le nom du fichier.", "ファイル名を変えます"], ["Internet est lent.", "インターネットが遅いです"], ["J'entends mal le son.", "音がよく聞こえません"],
    ["Les lettres sont trop petites.", "文字が小さすぎます"], ["Je vais enregistrer encore une fois.", "もう一度保存してみます"], ["Il reste encore un peu avant l'objectif d'aujourd'hui.", "今日の目標まで少し残っています"],
    ["Je vais rendre cet écran un peu plus lisible.", "この画面を少し見やすくします"], ["Je vais réécouter l'audio.", "もう一度音声を聞きます"], ["La prochaine fois, je m'entraînerai avec une phrase courte.", "次は短い文で練習します"],
    ["Je vais marquer les mots que je ne comprends pas.", "分からない単語に印を付けます"], ["Je vais finir d'abord la révision d'aujourd'hui.", "今日の復習を先に終わらせます"], ["Je vais réfléchir sans regarder la réponse.", "答えを見ずに考えます"],
    ["Je vais reformuler l'exemple avec mes propres mots.", "例文を自分の言葉に変えます"], ["Je vais vérifier si c'est bien enregistré.", "保存できたか確認します"], ["Je vais noter pourquoi je me suis trompé.", "間違えた理由をメモします"],
    ["Je vais dire la même expression à voix haute.", "同じ表現を声に出します"], ["Si je n'entends pas bien, je ralentirai la vitesse.", "聞こえないときは速度を下げます"], ["Faites remonter l'écran, s'il vous plaît.", "画面を上に戻してください"],
    ["À la fin, je reverrai les mots difficiles.", "最後に苦手な単語を見直します"], ["Je vais m'arrêter ici pour aujourd'hui.", "今日の学習をここで終わります"]
  ],
  business: [
    ["Je partagerai le compte rendu aujourd'hui.", "議事録を今日中に共有します"], ["Envoyez la dernière version des documents.", "資料の最新版を送ってください"], ["Je vérifierai d'abord avec le responsable.", "担当者に先に確認します"],
    ["Je vais résumer brièvement la réponse au client.", "顧客への回答は短く整理します"], ["Il manque un point dans les conditions du contrat.", "契約条件に抜けがあります"], ["J'ajusterai pour ne pas dépasser le budget.", "予算を超えないよう調整します"],
    ["S'il manque du stock, je chercherai une autre solution.", "在庫が足りない場合は代案を探します"], ["Je ferai une dernière vérification avant la publication.", "公開前に最終確認します"], ["S'il y a un problème, je le partagerai tout de suite.", "問題が起きたらすぐ共有します"]
  ]
};

const wordTargetCounts = targetCountsFrom(vocabulary);
const phraseTargetCounts = targetCountsFrom(phrases);

const phraseLikeWordLabels = new Set([
  "分かりません",
  "問題ありません",
  "お願いします",
  "お願いします / お手数ですが",
  "ゆっくり話す",
  "予約を変更する",
  "静かな部屋",
  "カードは使えますか"
]);

function expandAllowedLabel(label) {
  return String(label || "")
    .split(/\s*\/\s*/)
    .map((part) => part.trim())
    .filter(Boolean);
}

function isPhraseLikeWordLabel(japanese) {
  const label = String(japanese || "").trim();
  return phraseLikeWordLabels.has(label)
    || /^(新しい|小さい|大きい|きれいな|静かな|安い|高い|安全な|急ぎの|最初の|次の|近くの|少しの).+/.test(label)
    || /の(近く|前|後ろ|横|中|外|入口|出口|場所|写真|周り|向かい|準備|一覧|利用時間|手順|確認|修正|共有|検討|調整|管理|報告|記録|優先順位|意味|発音|練習|復習|例文|説明|質問|注意点|見直し|使い方)$/.test(label)
    || /を(利用する|見せる|見せます|受け取る|受け取りました|預ける|預けたいです|お願いします|ください|予約したいです|変更したいです|支払う|交換する|返金する|乗り換える|注文する|借りる)$/.test(label)
    || /.+を.+する$/.test(label)
    || /.+は.+(ですか|ますか)$/.test(label)
    || /で(払えますか|支払えますか)$/.test(label)
    || /(です|ます|ました|ください|できます|できません|しました|しています|したいです|ありますか|どこですか|いくらですか)$/.test(label);
}

const multilingualSafeWordRows = {
  basic: [
    ["心", "마음", "心", "le cœur"], ["体", "몸", "身体", "le corps"], ["頭", "머리", "头", "la tête"],
    ["目", "눈", "眼睛", "l'œil"], ["耳", "귀", "耳朵", "l'oreille"], ["口", "입", "嘴", "la bouche"],
    ["手", "손", "手", "la main"], ["足", "발", "脚", "le pied"], ["声", "목소리", "声音", "la voix"],
    ["言葉", "말", "话", "la parole"], ["国", "나라", "国家", "le pays"], ["町", "마을", "城镇", "la ville"],
    ["村", "촌", "村庄", "le village"], ["川", "강", "河", "la rivière"], ["海", "바다", "海", "la mer"],
    ["山", "산", "山", "la montagne"], ["空", "하늘", "天空", "le ciel"], ["風", "바람", "风", "le vent"],
    ["火", "불", "火", "le feu"], ["花", "꽃", "花", "la fleur"], ["木", "나무", "树", "l'arbre"],
    ["犬", "개", "狗", "le chien"], ["猫", "고양이", "猫", "le chat"], ["鳥", "새", "鸟", "l'oiseau"],
    ["肉", "고기", "肉", "la viande"], ["野菜", "채소", "蔬菜", "le légume"], ["果物", "과일", "水果", "le fruit"],
    ["砂糖", "설탕", "糖", "le sucre"], ["塩", "소금", "盐", "le sel"], ["油", "기름", "油", "l'huile"],
    ["皿", "접시", "盘子", "l'assiette"], ["スプーン", "숟가락", "勺子", "la cuillère"], ["ベッド", "침대", "床", "le lit"],
    ["鍵", "열쇠", "钥匙", "la clé"], ["財布", "지갑", "钱包", "le portefeuille"], ["時計", "시계", "手表", "la montre"],
    ["傘", "우산", "伞", "le parapluie"], ["帽子", "모자", "帽子", "le chapeau"], ["右", "오른쪽", "右边", "la droite"],
    ["左", "왼쪽", "左边", "la gauche"], ["上", "위", "上面", "le haut"], ["下", "아래", "下面", "le bas"],
    ["外", "밖", "外面", "l'extérieur"], ["遠く", "멀리", "远处", "loin"]
  ],
  practical: [
    ["状態", "상태", "状态", "l'état"], ["場合", "경우", "情况", "le cas"], ["目的", "목적", "目的", "le but"],
    ["結果", "결과", "结果", "le résultat"], ["原因", "원인", "原因", "la cause"], ["影響", "영향", "影响", "l'influence"],
    ["可能", "가능", "可能", "possible"], ["不可能", "불가능", "不可能", "impossible"], ["十分", "충분", "足够", "suffisant"],
    ["不足", "부족", "不足", "le manque"], ["余裕", "여유", "余裕", "la marge"], ["順番", "순서", "顺序", "l'ordre"],
    ["期限", "기한", "期限", "le délai"], ["最新", "최신", "最新", "récent"], ["以前", "이전", "以前", "avant"],
    ["次回", "다음번", "下次", "la prochaine fois"], ["全体", "전체", "整体", "l'ensemble"], ["一部", "일부", "一部分", "une partie"],
    ["便利", "편리", "方便", "pratique"], ["不便", "불편", "不方便", "peu pratique"], ["安全", "안전", "安全", "la sécurité"],
    ["危険", "위험", "危险", "le danger"], ["簡単", "간단", "简单", "simple"], ["複雑", "복잡", "复杂", "complexe"],
    ["自然", "자연스러움", "自然", "naturel"], ["正確", "정확", "准确", "exact"], ["無料", "무료", "免费", "gratuit"],
    ["有料", "유료", "收费", "payant"], ["原因", "원인", "原因", "la cause"], ["対策", "대책", "对策", "la mesure"]
  ],
  business: [
    ["目標", "목표", "目标", "l'objectif"], ["方針", "방침", "方针", "la politique"], ["戦略", "전략", "战略", "la stratégie"],
    ["計画", "계획", "计划", "le plan"], ["実績", "실적", "业绩", "les résultats"], ["分析", "분석", "分析", "l'analyse"],
    ["課題", "과제", "课题", "le problème"], ["要件", "요건", "需求", "l'exigence"], ["仕様", "사양", "规格", "la spécification"],
    ["納期", "납기", "交期", "le délai de livraison"], ["単価", "단가", "单价", "le prix unitaire"], ["利益", "이익", "利润", "le profit"],
    ["損失", "손실", "损失", "la perte"], ["部署", "부서", "部门", "le service"], ["役割", "역할", "角色", "le rôle"],
    ["責任", "책임", "责任", "la responsabilité"], ["権限", "권한", "权限", "l'autorisation"], ["連携", "연계", "协作", "la coopération"],
    ["交渉", "협상", "谈判", "la négociation"], ["合意", "합의", "同意", "l'accord"], ["保留", "보류", "保留", "en attente"],
    ["至急", "긴급", "紧急", "urgent"], ["通常", "일반", "通常", "normal"], ["社内", "사내", "公司内部", "interne"],
    ["社外", "사외", "公司外部", "externe"]
  ],
  travel: [
    ["観光", "관광", "观光", "le tourisme"], ["観光地", "관광지", "景点", "le site touristique"], ["広場", "광장", "广场", "la place"],
    ["橋", "다리", "桥", "le pont"], ["港", "항구", "港口", "le port"], ["海辺", "해변", "海边", "la plage"],
    ["温泉", "온천", "温泉", "la source chaude"], ["美術館", "미술관", "美术馆", "le musée d'art"], ["博物館", "박물관", "博物馆", "le musée"],
    ["劇場", "극장", "剧院", "le théâtre"], ["売店", "매점", "小卖部", "le kiosque"], ["屋台", "노점", "小吃摊", "le stand"],
    ["朝食", "아침 식사", "早餐", "le petit-déjeuner"], ["昼食", "점심", "午餐", "le déjeuner"], ["夕食", "저녁 식사", "晚餐", "le dîner"],
    ["辛い", "맵다", "辣", "épicé"], ["甘い", "달다", "甜", "sucré"], ["寒い", "춥다", "冷", "froid"],
    ["暑い", "덥다", "热", "chaud"], ["遅延", "지연", "延误", "le retard"], ["満席", "만석", "满座", "complet"],
    ["片道", "편도", "单程", "l'aller simple"], ["往復", "왕복", "往返", "l'aller-retour"], ["乗車券", "승차권", "车票", "le billet"],
    ["領収書", "영수증", "收据", "le reçu"]
  ]
};

const frenchSafeWordAliases = new Map([
  ["la prochaine fois", "prochainement"],
  ["peu pratique", "impratique"],
  ["le délai de livraison", "échéance"],
  ["le prix unitaire", "tarif"],
  ["en attente", "attente"],
  ["le site touristique", "attraction"],
  ["la source chaude", "thermes"],
  ["le musée d'art", "galerie"],
  ["l'aller simple", "aller-simple"]
]);

function normalizeSafeWordTarget(target, language) {
  let value = String(target || "").trim();
  if (!value) {
    return "";
  }

  if (language === "ko") {
    return value.replace(/\s+/g, "");
  }

  if (language !== "fr") {
    return value;
  }

  value = frenchSafeWordAliases.get(value) || value;
  value = value.replace(/^(le|la|les|un|une)\s+/i, "");
  value = value.replace(/^l'/i, "");
  value = value.replace(/^d'/i, "");

  return value.trim().split(/\s+/)[0] || "";
}

function safeWordRowsForLanguage(language) {
  const index = { ko: 1, zh: 2, fr: 3 }[language];
  return Object.fromEntries(Object.entries(multilingualSafeWordRows).map(([key, rows]) => [
    key,
    rows
      .map((row) => [normalizeSafeWordTarget(row[index], language), row[0]])
      .filter((row) => row[0])
  ]));
}

const koreanWordBoost7 = safeWordRowsForLanguage("ko");
const chineseWordBoost7 = safeWordRowsForLanguage("zh");
const frenchWordBoost7 = safeWordRowsForLanguage("fr");

const wordAllowedLabels = Object.fromEntries(Object.entries(vocabulary).map(([key, category]) => [
  key,
  new Set(category.words
    .flatMap((item) => [item.japanese, ...expandAllowedLabel(item.japanese)])
    .filter((label) => !isPhraseLikeWordLabel(label)))
]));

function alignWordGroupsToEnglish(groups) {
  return Object.fromEntries(Object.entries(groups).map(([key, rows]) => [
    key,
    rows.filter((row) => !isPhraseLikeWordLabel(row[1]))
  ]));
}

const expandedKoreanWords = topUpLanguageGroups("ko", "word", alignWordGroupsToEnglish(mergeGroups(mergeGroups(mergeGroups(mergeGroups(mergeGroups(mergeGroups(mergeGroups(koreanWords, koreanWordBoost), koreanWordBoost2), koreanWordBoost3), koreanWordBoost4), koreanWordBoost5), koreanWordBoost6), koreanWordBoost7)), wordTargetCounts, { allowedWordLabels: wordAllowedLabels });
const expandedKoreanPhrases = topUpLanguageGroups("ko", "phrase", mergeGroups(mergeGroups(mergeGroups(mergeGroups(mergeGroups(koreanPhrases, koreanPhraseBoost), koreanPhraseBoost2), koreanPhraseBoost3), koreanPhraseBoost4), koreanPhraseBoost5), phraseTargetCounts);
const expandedChineseWords = topUpLanguageGroups("zh", "word", alignWordGroupsToEnglish(mergeGroups(mergeGroups(mergeGroups(mergeGroups(mergeGroups(mergeGroups(mergeGroups(chineseWords, chineseWordBoost), chineseWordBoost2), chineseWordBoost3), chineseWordBoost4), chineseWordBoost5), chineseWordBoost6), chineseWordBoost7)), wordTargetCounts, { allowedWordLabels: wordAllowedLabels });
const expandedChinesePhrases = topUpLanguageGroups("zh", "phrase", mergeGroups(mergeGroups(mergeGroups(mergeGroups(mergeGroups(chinesePhrases, chinesePhraseBoost), chinesePhraseBoost2), chinesePhraseBoost3), chinesePhraseBoost4), chinesePhraseBoost5), phraseTargetCounts);
const expandedFrenchWords = topUpLanguageGroups("fr", "word", alignWordGroupsToEnglish(mergeGroups(mergeGroups(mergeGroups(mergeGroups(mergeGroups(mergeGroups(mergeGroups(frenchWords, frenchWordBoost), frenchWordBoost2), frenchWordBoost3), frenchWordBoost4), frenchWordBoost5), frenchWordBoost6), frenchWordBoost7)), wordTargetCounts, { allowedWordLabels: wordAllowedLabels });
const expandedFrenchPhrases = topUpLanguageGroups("fr", "phrase", mergeGroups(mergeGroups(mergeGroups(mergeGroups(mergeGroups(frenchPhrases, frenchPhraseBoost), frenchPhraseBoost2), frenchPhraseBoost3), frenchPhraseBoost4), frenchPhraseBoost5), phraseTargetCounts);
const englishWords = decorateCollection(vocabulary, "word", "en");
const englishPhrases = decorateCollection(phrases, "phrase", "en");

export const languagePacks = {
  en: {
    label: "英語",
    shortLabel: "英",
    targetName: "英語",
    speechLang: "en-US",
    datasets: {
      word: englishWords,
      phrase: englishPhrases
    }
  },
  ko: {
    label: "韓国語",
    shortLabel: "韓",
    targetName: "韓国語",
    speechLang: "ko-KR",
    datasets: {
      word: createCollection(expandedKoreanWords, "word", "ko"),
      phrase: createCollection(expandedKoreanPhrases, "phrase", "ko")
    }
  },
  zh: {
    label: "中国語",
    shortLabel: "中",
    targetName: "中国語",
    speechLang: "zh-CN",
    datasets: {
      word: createCollection(expandedChineseWords, "word", "zh"),
      phrase: createCollection(expandedChinesePhrases, "phrase", "zh")
    }
  },
  fr: {
    label: "フランス語",
    shortLabel: "仏",
    targetName: "フランス語",
    speechLang: "fr-FR",
    datasets: {
      word: createCollection(expandedFrenchWords, "word", "fr"),
      phrase: createCollection(expandedFrenchPhrases, "phrase", "fr")
    }
  }
};

